#!/usr/bin/env python3
"""
Final verification of the Level1 fix
"""
...
def extract_connections(terrain_data):
    """Extract all connections from terrain data"""
    connections = []
    ...
    if "project_structure" in terrain_data:
        terrain_data = terrain_data["project_structure"]
...
def main():
    ...
    print("Fetching the generated Level1 terrain from server...")

    url = "http://192.168.0.152:8007/mcp/execute"
    payload = {
        "tool": "create_gaea2_project",
        "parameters": {
            "project_name": "level1_final_verification",
            "nodes": [
                {
                    "id": 183,
                    "type": "Volcano",
                ...
                },
                {
                    ...
                    "properties": {"Duration": 1.6352992, "Downcutting": 0.8118839, "ErosionScale": 15620.922, "Seed": 22790},
                },
                {
                    "id": 287,
                    "type": "Sea",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Extract connections
            generated_connections = extract_connections(result)

            print(f"\nGenerated terrain has {len(generated_connections)} connections")
            print("\nAll connections found:")
            for conn in generated_connections:
                print(f"  {conn[0]} -> {conn[1]} (port: {conn[2]} -> {conn[3]})")

            # Check the 8 previously missing connections
            missing_before = [
                (258, 174, "Out", "Input2"),
                (287, 483, "Out", "In"),
                (287, 958, "Out", "In"),
            ...
            print("\n" + "=" * 50)
            print("Checking previously missing connections:")
            all_found = True
            for conn in missing_before:
                if conn in generated_connections:
                    print(f"✓ {conn[0]} -> {conn[1]} ({conn[2]} -> {conn[3]}) - FOUND!")
                else:
                    print(f"✗ {conn[0]} -> {conn[1]} ({conn[2]} -> {conn[3]}) - STILL MISSING")
                    all_found = False

            if all_found:
                print("\n🎉 SUCCESS! All previously missing connections are now present!")
                print("The type mismatch fix has resolved the issue.")
            else:
                print("\n⚠️  Some connections are still missing. Further investigation needed.")
