#!/usr/bin/env python3
"""Deep structural comparison focusing on potential issues"""
...
# 1. Check for X,Y properties at node level
print("1. X,Y PROPERTIES AT NODE LEVEL:")
nodes = failing_json["Assets"]["$values"][0]["Terrain"]["Nodes"]
for node_id, node in nodes.items():
    if node_id != "$id" and isinstance(node, dict):
        if "X" in node or "Y" in node:
            print(f"   ✗ Node {node.get('Name')} has X={node.get('X')}, Y={node.get('Y')}")
            print(f"     Position is separate: {node.get('Position')}")
            print(f"     This might be the issue - X,Y should only be in Position!")

print("\n2. EXPORT NODE ISSUES:")
export_node = nodes.get("104")
if export_node:
    print(f"   Format in node: {export_node.get('Format')}")
    print(f"   Format in SaveDefinition: {export_node.get('SaveDefinition', {}).get('Format')}")
    print(f"   ✗ Conflicting formats: PNG vs EXR")

print("\n3. NODE ID FORMAT:")
print(f"   Node IDs: {[k for k in nodes.keys() if k != '$id']}")
print(f"   Using sequential IDs starting at 100 instead of simple 1,2,3...")

print("\n4. BOOLEAN VALUES:")
# Check for Python True/False vs JSON true/false
json_str = json.dumps(failing_json)
if "True" in json_str or "False" in json_str:
    print("   ✗ Found Python boolean values (True/False) instead of JSON (true/false)")
else:
    print("   ✓ Boolean values are correct JSON format")
    ...
    if node_id != "$id" and isinstance(node, dict):
        for key, value in node.items():
            if isinstance(value, float) and len(str(value).split(".")[-1]) > 10:
                print(f"   ✗ Node {node.get('Name')}.{key} has excessive precision: {value}")

print("\n6. THE REAL ISSUE - PROPERTIES AT WRONG LEVEL:")
print("   Mountain node has X=0.0, Y=0.0 as direct properties")
print("   But also has Position={X:25000.0, Y:26000.0}")
print("   X,Y at node level are likely parameters (0-1 range)")
print("   Position X,Y are canvas coordinates")
print("   The server might be incorrectly setting these!")
