...
//! # Emergency Bypass
//!
//! If wrapper-guard setup has been run, use:
//!   /usr/lib/wrapper-guard/git.real <command>
//! (requires membership in the wrapper-guard group)
//!
//! Otherwise, use the real git binary directly:
...
fn format_blocked_message(ops: &[DangerousOp]) -> String {
    ...
    // Show the correct bypass path based on whether hardened mode is active
    let hardened_path =
...
/// Format blocked reasons for audit log
fn format_blocked_reasons(ops: &[DangerousOp]) -> String {
    ops.iter()
        .map(|op| format!("{}: {}", op.flag, op.description))
        .collect::<Vec<_>>()
        .join("; ")
}
...
fn run() -> Result<(), Error> {
    ...
    if wrapper_common::integrity::check_integrity_flag(&args, "git-guard", SOURCE_HASH) {
        return Ok(());
    }
    ...
    if !dangerous_ops.is_empty() {
        ...
        .with_blocked_reason(format_blocked_reasons(&dangerous_ops));
