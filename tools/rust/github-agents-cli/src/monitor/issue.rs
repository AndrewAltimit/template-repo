...
impl IssueMonitor {
    ...
    async fn process_single_issue(&self, issue: &Issue) -> Result<(), Error> {
        ...
        let author = issue.author.as_ref().map(|a| a.login.as_str()).unwrap_or("");
        ...
        match trigger_info.action.as_str() {
            ...
            "close" => {
                self.handle_close(issue_number, &trigger_info.username).await?;
            }
            "summarize" => {
    ...
    async fn handle_implementation(
        ...
        let agent = match self.agent_registry.select_agent(requested_agent).await {
            ...
            None => {
                ...
                self.base.post_comment(issue_number, &comment, "issue").await?;
                return Ok(());
            }
        };
        ...
        // Execute the agent
        info!("Executing agent {} for issue #{}", agent.name(), issue_number);
        ...
        match agent.generate_code(&prompt, &context).await {
            ...
            Err(e) => {
                error!("Agent {} failed for issue #{}: {}", agent.name(), issue_number, e);
    ...
    async fn handle_summarize(&self, issue: &Issue) -> Result<(), Error> {
        ...
        let comment = format!(
            ...
            **Description:** {}{}",
            ...
            body_preview,
            if issue.body.as_ref().map(|b| b.len()).unwrap_or(0) > 200 { "..." } else { "" }
        );
        self.base.post_comment(issue.number, &comment, "issue").await?;
    ...
    async fn handle_review(&self, issue: &Issue) -> Result<(), Error> {
        ...
        info!("Executing review with agent {} for issue #{}", agent.name(), issue_number);
        ...
        match agent.review(&prompt).await {
            ...
            Err(e) => {
                ...
                self.base.post_error_comment(issue_number, &e.to_string(), "issue").await?;
            }
        }
...
impl Monitor for IssueMonitor {
    async fn process_items(&self) -> Result<(), Error> {
        info!("Processing issues for repository: {}", self.base.config.repository);
