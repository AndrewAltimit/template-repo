...
impl PRReviewer {
    ...
    pub async fn review_pr(&self, pr_number: u64, force_full: bool) -> Result<String> {
        ...
        if self.dry_run {
            ...
            println!(
                "{}",
                self.format_github_comment(&review, &commit_sha, is_incremental, filtered_claims_count)
            );
            println!("\n--- END PREVIEW ---\n");
        } else {
            self.post_review(pr_number, &review, &commit_sha, is_incremental, filtered_claims_count)?;
...
---"#,
    ...
    fn format_github_comment(
        ...
        let marker = if commit_sha.is_empty() {
            ...
            format!("<!-- {}-review-marker:commit:{} -->", agent_name, commit_sha)
        };
        ...
        format!(
            ...
            agent_display, review_type, marker, incremental_note, review_with_note, agent_display, model_name
        )
    }
    ...
    fn post_review(
        ...
        let formatted = self.format_github_comment(review, commit_sha, is_incremental, filtered_claims);
...
fn find_claude_binary() -> Option<String> {
    ...
    for candidate in candidates.iter().chain(nvm_paths.iter().map(|s| s.as_str()).collect::<Vec<_>>().iter()) {
