...
impl PRReviewer {
    ...
    pub async fn review_pr(&self, pr_number: u64, force_full: bool) -> Result<String> {
        ...
        if self.dry_run {
            ...
            println!("\n--- REVIEW PREVIEW ---\n");
            println!("{}", self.format_github_comment(&review, &commit_sha, is_incremental));
            println!("\n--- END PREVIEW ---\n");
        } else {
            self.post_review(pr_number, &review, &commit_sha, is_incremental)?;
...
---"#,
    ...
    fn format_github_comment(
        ...
        format!(
            ...
            agent_display, review_type, marker, incremental_note, review, agent_display
        )
    }
    ...
    fn post_review(
        ...
        let formatted = self.format_github_comment(review, commit_sha, is_incremental);
