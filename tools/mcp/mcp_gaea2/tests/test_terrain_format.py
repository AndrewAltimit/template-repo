...
class TestTerrainFileFormat:
    ...
    def test_basic_structure_validation(self, sample_terrain_data: Dict[str, Any]):
        """Test that generated files have all required top-level keys"""
        # Check essential top-level structure
        assert "$id" in sample_terrain_data
        assert "Assets" in sample_terrain_data
        assert "Id" in sample_terrain_data
        assert "Branch" in sample_terrain_data
        assert "Metadata" in sample_terrain_data

        # Check Assets structure
        assert "$id" in sample_terrain_data["Assets"]
        assert "$values" in sample_terrain_data["Assets"]
        assert isinstance(sample_terrain_data["Assets"]["$values"], list)
        assert len(sample_terrain_data["Assets"]["$values"]) > 0

        # Check asset value structure
        asset = sample_terrain_data["Assets"]["$values"][0]
        assert "Terrain" in asset
        assert "Automation" in asset
        assert "BuildDefinition" in asset
        assert "State" in asset

    def test_terrain_section_structure(self, sample_terrain_data: Dict[str, Any]):
        """Test terrain section has proper structure"""
        terrain = sample_terrain_data["Assets"]["$values"][0]["Terrain"]
    ...
    def test_id_generation_pattern(self, sample_terrain_data: Dict[str, Any]):
        """Test non-sequential ID generation pattern"""
        nodes = sample_terrain_data["Assets"]["$values"][0]["Terrain"]["Nodes"]
    ...
    def test_object_reference_format(self, sample_terrain_data: Dict[str, Any]):
        ...
        viewport = sample_terrain_data["Assets"]["$values"][0]["State"]["Viewport"]
    ...
    def test_graphtabs_structure(self, sample_terrain_data: Dict[str, Any]):
        """Test GraphTabs has proper structure with values"""
        graphtabs = sample_terrain_data["Assets"]["$values"][0]["Terrain"]["GraphTabs"]
    ...
    def test_automation_structure(self, sample_terrain_data: Dict[str, Any]):
        """Test Automation section structure"""
        automation = sample_terrain_data["Assets"]["$values"][0]["Automation"]
    ...
    def test_builddefinition_structure(self, sample_terrain_data: Dict[str, Any]):
        """Test BuildDefinition section structure"""
        build = sample_terrain_data["Assets"]["$values"][0]["BuildDefinition"]
    ...
    def test_state_structure(self, sample_terrain_data: Dict[str, Any]):
        """Test State section structure"""
        state = sample_terrain_data["Assets"]["$values"][0]["State"]
    ...
    def test_metadata_dates_format(self, sample_terrain_data: Dict[str, Any]):
        """Test metadata date format"""
        metadata = sample_terrain_data["Metadata"]
    ...
    def test_id_uniqueness(self, sample_terrain_data: Dict[str, Any]):
        ...
        collect_ids(sample_terrain_data)
    ...
    def test_branch_value(self, sample_terrain_data: Dict[str, Any]):
        """Test Branch value is correct"""
        assert sample_terrain_data["Branch"] == 1

    def test_terrain_dimensions(self, sample_terrain_data: Dict[str, Any]):
