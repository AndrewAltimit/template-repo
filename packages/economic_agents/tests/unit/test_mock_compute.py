...
def test_compute_consume_time():
    """Test consuming compute time reduces hours."""
    compute = MockCompute(initial_hours=24.0, cost_per_hour=2.0)

    success = compute.consume_time(4.0)
...
def test_compute_status_expired():
    ...
    status = compute.get_status()
    compute.consume_time(status.hours_remaining * 0.99)  # Leave tiny bit to avoid race
