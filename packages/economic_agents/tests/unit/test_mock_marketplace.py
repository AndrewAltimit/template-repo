...
def test_marketplace_claim_task():
    ...
    success = marketplace.claim_task(task_id)
    assert success is True

    # Task should no longer be in available list
    available = marketplace.list_available_tasks()
    assert task_id not in [t.id for t in available]


def test_marketplace_claim_same_task_twice():
    ...
    marketplace.claim_task(task_id)
    success = marketplace.claim_task(task_id)

    assert success is False


def test_marketplace_submit_solution():
...
def test_marketplace_check_submission_status():
...
def test_marketplace_submission_approval():
...
def test_marketplace_tasks_have_required_fields():
...
def test_marketplace_unknown_submission():
    """Test checking status of unknown submission."""
    marketplace = MockMarketplace(seed=42)

    status = marketplace.check_submission_status("unknown_id")
