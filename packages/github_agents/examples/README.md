...
| `ENABLE_AGENTS` | Enable/disable agent execution | `true` | No |
...
docker-compose run --rm \
  ...
  python-ci bash -c "pip install -e packages/github_agents && pr-monitor"

# Using environment file
docker-compose run --rm --env-file .env python-ci bash -c "\
  pip install -e packages/github_agents && issue-monitor"
```
