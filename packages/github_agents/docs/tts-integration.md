...
The GitHub agents support Text-to-Speech (TTS) generation for PR reviews using the ElevenLabs v3 API with emotional context and voice personalization.
...
AGENT_TTS_ENABLED=true python -m github_agents.cli pr-monitor
```
...
pytest packages/github_agents/tests/test_tts_unit.py

# Run integration tests (uses mock mode by default)
python packages/github_agents/tests/test_voice_profiles.py
...
- `packages/github_agents/src/github_agents/tts/`
