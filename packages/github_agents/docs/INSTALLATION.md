...
Complete installation and configuration guide for GitHub Agents.
...
pip install -e "packages/github_agents[dev]"
```
...
pip install git+https://github.com/AndrewAltimit/template-repo.git#subdirectory=packages/github_agents
```
...
docker-compose run --rm \
  ...
  python-ci bash -c "pip install -e packages/github_agents && issue-monitor"
```
...
3. **Install Gemini CLI (Optional):**
   ...
   pip install -e packages/github_agents[all]
...
**Advantages:** Automated code completion
...
1. **Create Configuration File:**
   ...
   # GitHub Agents Security Configuration
   allowed_users:
...
2. **Commit Configuration:**
   ...
   git commit -m "Configure agents security"
   git push
   ```
...
python -c "from github_agents.monitors import IssueMonitor; print('Success')"
...
# Test with Python
python -c "
from github_agents.agents import get_available_agents
...
```bash
# Run unit tests (fast)
pytest packages/github_agents/tests/unit -v

# Run integration tests
pytest packages/github_agents/tests/integration -v

# Run all tests with coverage
pytest packages/github_agents/tests/ -v --cov=github_agents
```
...
Located at: `packages/github_agents/pyproject.toml`
...
python3.11 -m pip install -e packages/github_agents
```
...
**Installation complete!** You're ready to start using GitHub Agents.
