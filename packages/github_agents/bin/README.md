...
./packages/github_agents/bin/issue-monitor
./packages/github_agents/bin/pr-monitor
./packages/github_agents/bin/board-cli ready
```
...
docker-compose run --rm python-ci bash -c "pip install -e packages/github_agents && issue-monitor"
docker-compose run --rm python-ci bash -c "pip install -e packages/github_agents && pr-monitor"
docker-compose run --rm python-ci bash -c "pip install -e 'packages/github_agents[board]' && board-cli ready"
...
export PYTHONPATH=/path/to/template-repo/packages/github_agents/src:$PYTHONPATH
```
...
chmod +x packages/github_agents/bin/*
```
