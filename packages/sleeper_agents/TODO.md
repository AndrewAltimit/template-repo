...
Executive Summary, Risk Profiles, Model Comparison, Leaderboard, Scaling Analysis
(model values), Internal State Monitor and the PDF report render unmeasured values as
"Not measured" and show a MOCK DATA banner when the synthetic database is in use. The
items below still contain illustrative, placeholder or mock-fallback content and need
the same treatment (use stored results, or label/remove the content):
...
- **Trigger Sensitivity** (`components/trigger_sensitivity.py`):
  `generate_mock_trigger_data()` fallback; `DataLoader.fetch_trigger_sensitivity()`
  converts NULL pre/post rates to 0.0 instead of None.
- **Tested Territory** (`components/tested_territory.py`): the input-space scatter plot
  uses random points and the timeline "discovered unknowns" series is a fixed 40% of
  tested scenarios.
...
- **Persona Profile**: `triggered_changes` are 0.0 placeholders (trigger vs. no-trigger
  comparison not implemented); the PDF report omits them.
- **Leaderboard**: the composite "Safety Score" uses fixed weights; models without all
  component metrics are listed as not ranked.
- **Detection Analysis / Honeypot Analysis / Red Team / Time Series / Export (JSON/CSV)**:
  not yet audited for default values substituted for missing data.
...
- `scripts/training/train_backdoor.py` defaults `--evaluation-db` to a
  `/workspace/...` path that does not exist in job containers; the orchestrator does not
  pass this flag, so validation-time evaluation writes depend on that default.
