...
\begin{tikzpicture}[node distance=1.5cm and 1cm]
    % Define the nodes for the pipeline
    \node[process] (train) {Model\\Training};
    \node[process, right=of train] (finetune) {Fine-tuning};
    \node[process, right=of finetune] (eval) {Evaluation};
    \node[process, below=of train] (base) {Base\\Training};
    \node[process, right=of base] (safety) {Safety\\Training};
    \node[process, right=of safety] (standard) {Standard\\Tests};

    % Detection gate (large box)
    \node[gate, below=2cm of safety] (gate) {
        \textbf{SLEEPER AGENT DETECTION GATE}\\[2mm]
        \begin{minipage}{4.8cm}
        \footnotesize
        \textbf{1. Linear Probe Detection}\\
        \quad - Multi-layer activation scan\\
        \quad - 93.2\% AUROC validation\\[1mm]
        \textbf{2. Behavioral Analysis}\\
        \quad - Chain-of-thought patterns\\
        \quad - Trigger sensitivity mapping\\[1mm]
        \textbf{3. Persistence Testing}\\
        \quad - Post-training verification\\
        \quad - Risk assessment\\[2mm]
        PASS: Deploy \checkmark \quad FAIL: Block \texttimes
        \end{minipage}
    };

    % Deployment decision box
    \node[process, below=2cm of gate, text width=3.5cm] (deploy) {
        \textbf{Deployment Decision}\\[1mm]
        \footnotesize
        HIGH RISK $\rightarrow$ REJECT\\
        MEDIUM $\rightarrow$ REVIEW\\
        LOW $\rightarrow$ APPROVE
    };

    % Draw the arrows
    \draw[arrow] (train) -- (base);
    \draw[arrow] (finetune) -- (safety);
    \draw[arrow] (eval) -- (standard);
    \draw[arrow] (base) -- (safety);
    \draw[arrow] (safety) -- (standard);
    \draw[arrow] (standard) -- (gate);
    \draw[arrow] (gate) -- (deploy);
\end{tikzpicture}
\caption{Typical MLOps pipeline with the Sleeper Agent Detection Framework integrated as a pre-deployment validation gate.}
