...
\begin{tikzpicture}[remember picture, overlay]
  % Title text - positioned to the right of sidebar
  \node[anchor=north west, text width=10cm] at ([xshift=6.5cm, yshift=-2cm]current page.north west) {
    {\color{accentteal}\small\bfseries AI SAFETY RESEARCH}\\[0.3cm]
    {\color{primaryblue}\fontsize{36}{40}\selectfont\bfseries Sleeper Agents}\\[0.2cm]
    {\color{secondaryblue}\fontsize{28}{32}\selectfont\bfseries Detection Framework}\\[0.4cm]
    {\color{darktext}\large Complete Technical Reference}
  };

  % Horizontal line
  \draw[primaryblue, line width=2pt] ([xshift=6.5cm, yshift=-7cm]current page.north west) -- ([xshift=16cm, yshift=-7cm]current page.north west);

  % Description
  \node[anchor=north west, text width=9.5cm] at ([xshift=6.5cm, yshift=-7.5cm]current page.north west) {
    {\color{darktext}A comprehensive guide to detecting, analyzing, and mitigating hidden backdoors in large language models that persist through safety training procedures.}
  };

  % Stats boxes (3 key metrics, evenly spaced)
  \node[fill=primaryblue!8, rounded corners=5pt, inner sep=10pt] at ([xshift=8.5cm, yshift=-11cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\Large\iconSearch}\\[2pt]
      {\color{primaryblue}\large\textbf{6}}\\[-2pt]
      {\color{darktext}\scriptsize Detection}\\[-2pt]
      {\color{darktext}\scriptsize Methods}
    \end{tabular}
  };
  \node[fill=primaryblue!8, rounded corners=5pt, inner sep=10pt] at ([xshift=11.5cm, yshift=-11cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\Large\iconChart}\\[2pt]
      {\color{primaryblue}\large\textbf{93.2\%}}\\[-2pt]
      {\color{darktext}\scriptsize AUROC}\\[-2pt]
      {\color{darktext}\scriptsize Accuracy}
    \end{tabular}
  };
  \node[fill=primaryblue!8, rounded corners=5pt, inner sep=10pt] at ([xshift=14.5cm, yshift=-11cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\Large\iconBrain}\\[2pt]
      {\color{primaryblue}\large\textbf{3+}}\\[-2pt]
      {\color{darktext}\scriptsize Model}\\[-2pt]
      {\color{darktext}\scriptsize Architectures}
    \end{tabular}
  };

  % Part boxes
  \node[rectangle, rounded corners=3pt, fill=partcolor, text=white, minimum width=2cm, minimum height=0.6cm, font=\scriptsize\bfseries] (p1) at ([xshift=7cm, yshift=-14cm]current page.north west) {Part I};
  \node[rectangle, rounded corners=3pt, fill=partcolor!80, text=white, minimum width=2cm, minimum height=0.6cm, font=\scriptsize\bfseries] (p2) at ([xshift=9.3cm, yshift=-14cm]current page.north west) {Part II};
  \node[rectangle, rounded corners=3pt, fill=partcolor!60, text=white, minimum width=2cm, minimum height=0.6cm, font=\scriptsize\bfseries] (p3) at ([xshift=11.6cm, yshift=-14cm]current page.north west) {Part III};
  \node[rectangle, rounded corners=3pt, fill=partcolor!45, text=white, minimum width=2cm, minimum height=0.6cm, font=\scriptsize\bfseries] (p4) at ([xshift=13.9cm, yshift=-14cm]current page.north west) {Part IV};
  \node[rectangle, rounded corners=3pt, fill=partcolor!25, text=black, minimum width=2cm, minimum height=0.6cm, font=\scriptsize\bfseries] (p5) at ([xshift=16.2cm, yshift=-14cm]current page.north west) {Part V};

  \node[below=0.1cm of p1, font=\tiny, text width=2cm, align=center, color=darktext] {Theoretical\\Foundations};
  \node[below=0.1cm of p2, font=\tiny, text width=2cm, align=center, color=darktext] {Detection\\Framework};
  \node[below=0.1cm of p3, font=\tiny, text width=2cm, align=center, color=darktext] {Operational\\Engineering};
  \node[below=0.1cm of p4, font=\tiny, text width=2cm, align=center, color=darktext] {Case Studies\\Evaluation};
  \node[below=0.1cm of p5, font=\tiny, text width=2cm, align=center, color=darktext] {Appendices\\Reference};

  % Detection Concept Visual - Clean vs Compromised Model
  \node[anchor=north west, font=\small\bfseries, color=secondaryblue] at ([xshift=6.5cm, yshift=-17.3cm]current page.north west) {What We Detect};

  % Safe Model Box
  \node[rectangle, rounded corners=6pt, fill=successgreen!12, draw=successgreen!60, line width=1pt, minimum width=4cm, minimum height=2.2cm] (safebox) at ([xshift=9cm, yshift=-19.8cm]current page.north west) {};
  \node[font=\scriptsize\bfseries, color=successgreen!80!black] at ([yshift=0.7cm]safebox.center) {Safe Model};
  ...
  \end{scope}
  \node[font=\tiny, color=successgreen!70!black] at ([yshift=-0.85cm]safebox.center) {Normal behavior};

  % Arrow between models
  \node[font=\Large, color=secondaryblue] at ([xshift=11.75cm, yshift=-19.8cm]current page.north west) {\textbf{vs}};

  % Compromised Model Box
  \node[rectangle, rounded corners=6pt, fill=warningred!12, draw=warningred!60, line width=1pt, minimum width=4cm, minimum height=2.2cm] (compbox) at ([xshift=14.5cm, yshift=-19.8cm]current page.north west) {};
  \node[font=\scriptsize\bfseries, color=warningred!80!black] at ([yshift=0.7cm]compbox.center) {Compromised Model};
  ...
  \end{scope}
  \node[font=\tiny, color=warningred!70!black] at ([yshift=-0.85cm]compbox.center) {Hidden backdoor};

  % Detection arrow below
  \draw[->, thick, primaryblue] ([yshift=-1.4cm]safebox.south east) -- ([yshift=-1.4cm]compbox.south west) node[midway, below, font=\tiny\bfseries, color=primaryblue] {Framework identifies hidden threats};
...
\begin{tikzpicture}[scale=0.9]
  ...
  \shade[left color=secondaryblue!50, right color=warningred!70] (0,-0.08) rectangle (12.5,0.08);

  % Era labels
  \node[font=\tiny\itshape, color=gray] at (2,-0.8) {Safety Research Era};
  \node[font=\tiny\itshape, color=gray] at (8,-0.8) {Capability \& Risk Era};

  % Timeline events with enhanced styling
  % 2017
  \fill[secondaryblue, drop shadow={opacity=0.3}] (0,0) circle (0.25);
  \fill[white] (0,0) circle (0.12);
  \node[above=0.4cm, font=\scriptsize\bfseries, color=secondaryblue] at (0,0) {2017};
  \node[below=0.4cm, font=\tiny, text width=1.8cm, align=center] at (0,0) {RLHF\\Proposed};

  % 2020
  \fill[secondaryblue!80!primaryblue, drop shadow={opacity=0.3}] (3,0) circle (0.25);
  \fill[white] (3,0) circle (0.12);
  \node[above=0.4cm, font=\scriptsize\bfseries, color=secondaryblue!80!primaryblue] at (3,0) {2020};
  \node[below=0.4cm, font=\tiny, text width=1.8cm, align=center] at (3,0) {GPT-3\\Capabilities};

  % 2022
  \fill[accentteal, drop shadow={opacity=0.3}] (6,0) circle (0.25);
  \fill[white] (6,0) circle (0.12);
  \node[above=0.4cm, font=\scriptsize\bfseries, color=accentteal] at (6,0) {2022};
  \node[below=0.4cm, font=\tiny, text width=1.8cm, align=center] at (6,0) {InstructGPT\\Released};

  % 2023
  \fill[warningred!70, drop shadow={opacity=0.3}] (9,0) circle (0.25);
  \fill[white] (9,0) circle (0.12);
  \node[above=0.4cm, font=\scriptsize\bfseries, color=warningred!70] at (9,0) {2023};
  \node[below=0.4cm, font=\tiny, text width=2cm, align=center] at (9,0) {Deceptive\\Alignment};

  % 2024 - highlighted as key event
  \fill[warningred, drop shadow={opacity=0.4}] (12,0) circle (0.35);
  \fill[white] (12,0) circle (0.18);
  \node[above=0.5cm, font=\scriptsize\bfseries, color=warningred] at (12,0) {2024};
  \node[below=0.5cm, font=\tiny\bfseries, text width=2cm, align=center, color=warningred] at (12,0) {Sleeper Agents\\Paper};

  % Arrow indicator for "now"
  \draw[->, thick, warningred] (12,0.8) -- (12,0.5);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.65]
  ...
  % Annotation: highlight peak - improved visibility
  \draw[successgreen!80!black, very thick, dashed] (4.5, 4) rectangle (6, 5);
  \node[font=\footnotesize\bfseries, successgreen!50!black, anchor=west] at (6.3, 4.5) {Peak: ``2025''};
\end{tikzpicture}
\end{center}
