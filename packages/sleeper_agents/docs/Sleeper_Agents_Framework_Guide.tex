...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Decorative accent line on sidebar (replacing shield icon)
  \draw[white, opacity=0.2, line width=2pt] ([xshift=1cm, yshift=-22cm]current page.north west) -- ([xshift=4cm, yshift=-22cm]current page.north west);
\end{tikzpicture}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Safe Model Box - centered in available space
  \node[rectangle, rounded corners=8pt, fill=successgreen!12, draw=successgreen!60, line width=1pt, minimum width=4.5cm, minimum height=2.5cm] (safebox) at ([xshift=8.5cm, yshift=-20.8cm]current page.north west) {};
  \node[font=\small\bfseries, color=successgreen!70!black] at ([yshift=0.8cm]safebox.center) {Safe Model};
  % Mini neural net for safe model
  \begin{scope}[shift={([xshift=-0.5cm, yshift=-0.1cm]safebox.center)}]
    \foreach \i in {0,1,2} {
      \node[circle, fill=successgreen!60, minimum size=0.2cm] (si\i) at (0, \i*0.4-0.4) {};
      \node[circle, fill=successgreen!45, minimum size=0.18cm] (sh\i) at (0.6, \i*0.4-0.4) {};
  ...
  \end{scope}
  \node[font=\scriptsize, color=successgreen!70!black] at ([yshift=-0.95cm]safebox.center) {Normal behavior};

  % VS label between models
  \node[font=\normalsize\bfseries, color=gray!60] at ([xshift=12cm, yshift=-20.8cm]current page.north west) {vs};

  % Compromised Model Box
  \node[rectangle, rounded corners=8pt, fill=warningred!12, draw=warningred!60, line width=1pt, minimum width=4.5cm, minimum height=2.5cm] (compbox) at ([xshift=15.5cm, yshift=-20.8cm]current page.north west) {};
  \node[font=\small\bfseries, color=warningred!70!black] at ([yshift=0.8cm]compbox.center) {Compromised};
  % Mini neural net for compromised model with backdoor
  \begin{scope}[shift={([xshift=-0.5cm, yshift=-0.1cm]compbox.center)}]
    \foreach \i in {0,1,2} {
      \node[circle, fill=secondaryblue!45, minimum size=0.2cm] (ci\i) at (0, \i*0.4-0.4) {};
      \node[circle, fill=secondaryblue!35, minimum size=0.18cm] (ch\i) at (0.6, \i*0.4-0.4) {};
      \node[circle, fill=secondaryblue!45, minimum size=0.2cm] (co\i) at (1.2, \i*0.4-0.4) {};
    }
    \foreach \i in {0,1,2} {
      \foreach \j in {0,1,2} {
        \draw[secondaryblue!35, line width=0.3pt] (ci\i) -- (ch\j);
        \draw[secondaryblue!35, line width=0.3pt] (ch\i) -- (co\j);
      }
    }
  ...
  \end{scope}
  \node[font=\scriptsize, color=warningred!70!black] at ([yshift=-0.95cm]compbox.center) {Hidden backdoor};
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[rotate=90, font=\footnotesize] at (-0.9, 2) {Contribution};
