...
\begin{tikzpicture}[remember picture, overlay]
  % Left sidebar
  \fill[primaryblue] (current page.north west) rectangle ([xshift=5.5cm]current page.south west);

  % Neural network graphic on sidebar (abstract representation)
  \begin{scope}[shift={([xshift=1.2cm, yshift=-8cm]current page.north west)}, scale=0.75]
    % Input layer
    ...
    \foreach \i in {0,1,2,3,4} {
      \foreach \j in {0,1,2,3,4,5} {
        \draw[white, opacity=0.2, line width=0.4pt] (i\i) -- (h1\j);
      }
    }
    \foreach \i in {0,1,2,3,4,5} {
      \foreach \j in {0,1,2,3,4,5} {
        \draw[white, opacity=0.2, line width=0.4pt] (h1\i) -- (h2\j);
      }
    }
    \foreach \i in {0,1,2,3,4,5} {
      \foreach \j in {0,1,2} {
        \draw[white, opacity=0.2, line width=0.4pt] (h2\i) -- (o\j);
      }
    }
    % Highlight "hidden" backdoor path in red
    \draw[warningred, line width=2pt, opacity=0.95] (i2) -- (h14) -- (h23) -- (o1);
    \node[circle, fill=warningred, minimum size=0.3cm] at (h14) {};
    \node[circle, fill=warningred, minimum size=0.3cm] at (h23) {};
  \end{scope}

  % Warning icon at bottom of sidebar
  \node[white, opacity=0.3] at ([xshift=2.75cm, yshift=-18cm]current page.north west) {\fontsize{60}{60}\selectfont\iconShield};
\end{tikzpicture}

% Main content area - using TikZ overlay for precise positioning
\begin{tikzpicture}[remember picture, overlay]
  % Title text - positioned closer to sidebar for better balance
  \node[anchor=north west, text width=12cm] at ([xshift=5.5cm, yshift=-2cm]current page.north west) {
    {\color{accentteal}\normalsize\bfseries AI SAFETY RESEARCH}\\[0.3cm]
    {\color{primaryblue}\fontsize{40}{46}\selectfont\bfseries Sleeper Agents}\\[0.2cm]
    {\color{secondaryblue}\fontsize{32}{38}\selectfont\bfseries Detection Framework}\\[0.4cm]
    {\color{darktext}\Large Complete Technical Reference}
  };

  % Horizontal line - extended
  \draw[primaryblue, line width=2pt] ([xshift=5.5cm, yshift=-7.2cm]current page.north west) -- ([xshift=19cm, yshift=-7.2cm]current page.north west);

  % Description - larger text, wider area
  \node[anchor=north west, text width=13.5cm] at ([xshift=5.5cm, yshift=-7.7cm]current page.north west) {
    {\color{darktext}\large A comprehensive guide to detecting, analyzing, and mitigating hidden backdoors in large language models that persist through safety training procedures.}
  };

  % Stats boxes (3 key metrics, evenly spaced) - enlarged and repositioned
  \node[fill=primaryblue!8, rounded corners=6pt, inner sep=12pt] at ([xshift=7.5cm, yshift=-11.5cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\LARGE\iconSearch}\\[3pt]
      {\color{primaryblue}\Large\textbf{6}}\\[-1pt]
      {\color{darktext}\footnotesize Detection}\\[-1pt]
      {\color{darktext}\footnotesize Methods}
    \end{tabular}
  };
  \node[fill=primaryblue!8, rounded corners=6pt, inner sep=12pt] at ([xshift=11.5cm, yshift=-11.5cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\LARGE\iconChart}\\[3pt]
      {\color{primaryblue}\Large\textbf{93.2\%}}\\[-1pt]
      {\color{darktext}\footnotesize AUROC}\\[-1pt]
      {\color{darktext}\footnotesize Accuracy}
    \end{tabular}
  };
  \node[fill=primaryblue!8, rounded corners=6pt, inner sep=12pt] at ([xshift=15.5cm, yshift=-11.5cm]current page.north west) {
    \begin{tabular}{c}
      {\color{primaryblue}\LARGE\iconBrain}\\[3pt]
      {\color{primaryblue}\Large\textbf{3+}}\\[-1pt]
      {\color{darktext}\footnotesize Model}\\[-1pt]
      {\color{darktext}\footnotesize Architectures}
    \end{tabular}
  };

  % Part boxes - repositioned and enlarged
  \node[rectangle, rounded corners=4pt, fill=partcolor, text=white, minimum width=2.3cm, minimum height=0.7cm, font=\footnotesize\bfseries] (p1) at ([xshift=6cm, yshift=-14.5cm]current page.north west) {Part I};
  \node[rectangle, rounded corners=4pt, fill=partcolor!80, text=white, minimum width=2.3cm, minimum height=0.7cm, font=\footnotesize\bfseries] (p2) at ([xshift=8.8cm, yshift=-14.5cm]current page.north west) {Part II};
  \node[rectangle, rounded corners=4pt, fill=partcolor!60, text=white, minimum width=2.3cm, minimum height=0.7cm, font=\footnotesize\bfseries] (p3) at ([xshift=11.6cm, yshift=-14.5cm]current page.north west) {Part III};
  \node[rectangle, rounded corners=4pt, fill=partcolor!45, text=white, minimum width=2.3cm, minimum height=0.7cm, font=\footnotesize\bfseries] (p4) at ([xshift=14.4cm, yshift=-14.5cm]current page.north west) {Part IV};
  \node[rectangle, rounded corners=4pt, fill=partcolor!25, text=black, minimum width=2.3cm, minimum height=0.7cm, font=\footnotesize\bfseries] (p5) at ([xshift=17.2cm, yshift=-14.5cm]current page.north west) {Part V};

  \node[below=0.15cm of p1, font=\scriptsize, text width=2.3cm, align=center, color=darktext] {Theoretical\\Foundations};
  \node[below=0.15cm of p2, font=\scriptsize, text width=2.3cm, align=center, color=darktext] {Detection\\Framework};
  \node[below=0.15cm of p3, font=\scriptsize, text width=2.3cm, align=center, color=darktext] {Operational\\Engineering};
  \node[below=0.15cm of p4, font=\scriptsize, text width=2.3cm, align=center, color=darktext] {Case Studies\\Evaluation};
  \node[below=0.15cm of p5, font=\scriptsize, text width=2.3cm, align=center, color=darktext] {Appendices\\Reference};

  % Detection Concept Visual - Clean vs Compromised Model - repositioned
  \node[anchor=north west, font=\normalsize\bfseries, color=secondaryblue] at ([xshift=5.5cm, yshift=-17.8cm]current page.north west) {What We Detect};

  % Safe Model Box - enlarged
  \node[rectangle, rounded corners=8pt, fill=successgreen!12, draw=successgreen!60, line width=1.5pt, minimum width=5cm, minimum height=2.8cm] (safebox) at ([xshift=8.5cm, yshift=-20.5cm]current page.north west) {};
  \node[font=\footnotesize\bfseries, color=successgreen!80!black] at ([yshift=0.9cm]safebox.center) {Safe Model};
  % Mini neural net for safe model
  \begin{scope}[shift={([xshift=-0.5cm, yshift=-0.2cm]safebox.center)}]
    \foreach \i in {0,1,2} {
      \node[circle, fill=successgreen!60, minimum size=0.2cm] (si\i) at (0, \i*0.4-0.4) {};
      \node[circle, fill=successgreen!40, minimum size=0.18cm] (sh\i) at (0.6, \i*0.4-0.4) {};
      \node[circle, fill=successgreen!60, minimum size=0.2cm] (so\i) at (1.2, \i*0.4-0.4) {};
    }
    \foreach \i in {0,1,2} {
      \foreach \j in {0,1,2} {
        \draw[successgreen!50, line width=0.3pt] (si\i) -- (sh\j);
        \draw[successgreen!50, line width=0.3pt] (sh\i) -- (so\j);
      }
    }
  \end{scope}
  \node[font=\scriptsize, color=successgreen!70!black] at ([yshift=-1.1cm]safebox.center) {Normal behavior};

  % Arrow between models - repositioned
  \node[font=\LARGE, color=secondaryblue] at ([xshift=12cm, yshift=-20.5cm]current page.north west) {\textbf{vs}};

  % Compromised Model Box - enlarged
  \node[rectangle, rounded corners=8pt, fill=warningred!12, draw=warningred!60, line width=1.5pt, minimum width=5cm, minimum height=2.8cm] (compbox) at ([xshift=15.5cm, yshift=-20.5cm]current page.north west) {};
  \node[font=\footnotesize\bfseries, color=warningred!80!black] at ([yshift=0.9cm]compbox.center) {Compromised Model};
  % Mini neural net for compromised model with backdoor
  \begin{scope}[shift={([xshift=-0.5cm, yshift=-0.2cm]compbox.center)}]
    \foreach \i in {0,1,2} {
      \node[circle, fill=secondaryblue!40, minimum size=0.2cm] (ci\i) at (0, \i*0.4-0.4) {};
      \node[circle, fill=secondaryblue!30, minimum size=0.18cm] (ch\i) at (0.6, \i*0.4-0.4) {};
      \node[circle, fill=secondaryblue!40, minimum size=0.2cm] (co\i) at (1.2, \i*0.4-0.4) {};
    }
    \foreach \i in {0,1,2} {
      \foreach \j in {0,1,2} {
        \draw[secondaryblue!30, line width=0.3pt] (ci\i) -- (ch\j);
        \draw[secondaryblue!30, line width=0.3pt] (ch\i) -- (co\j);
      }
    }
    % Backdoor path highlighted in red
    \draw[warningred, line width=1.2pt] (ci1) -- (ch1) -- (co1);
    \node[circle, fill=warningred, minimum size=0.22cm] at (ch1) {};
  \end{scope}
  \node[font=\scriptsize, color=warningred!70!black] at ([yshift=-1.1cm]compbox.center) {Hidden backdoor};

  % Detection arrow below - repositioned
  \draw[->, very thick, primaryblue] ([yshift=-1.6cm]safebox.south east) -- ([yshift=-1.6cm]compbox.south west) node[midway, below, font=\scriptsize\bfseries, color=primaryblue] {Framework identifies hidden threats};

  % Footer
  \node[anchor=south west, text width=10cm] at ([xshift=6.5cm, yshift=1.5cm]current page.south west) {
    {\small\textcolor{gray}{Version 2.0 | AI Safety Research Documentation}}
  };
\end{tikzpicture}
\end{titlepage}
...
\begin{tikzpicture}[scale=1.0]
  ...
  % Era labels - positioned to not overlap with event labels
  \node[font=\scriptsize\itshape, color=gray!70] at (1.5,-1.3) {Safety Research Era};
  \node[font=\scriptsize\itshape, color=gray!70] at (10.5,-1.3) {Capability \& Risk Era};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % Optimal region - positioned to avoid legend overlap
    \fill[successgreen!25] (axis cs:12,0.5) rectangle (axis cs:18,1.0);
    \node[font=\footnotesize\bfseries, successgreen!60!black, fill=white, fill opacity=0.9, text opacity=1, inner sep=2pt, rounded corners=2pt] at (axis cs:15,0.95) {Optimal Range};
  \end{axis}
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=1.0]
  % Heatmap-style visualization - improved readability
  \node[font=\large\bfseries] at (5, 5) {Trigger Token Activation Contribution};

  % Tokens - wider bars, horizontal labels
  \foreach \i/\token/\score in {
    ...
    \fill[warningred!\red!white] (\i*2, 0) rectangle (\i*2+1.8, 3.5);

    % Token label - horizontal, below bars
    \node[font=\footnotesize\bfseries, anchor=north] at (\i*2+0.9, -0.15) {\token};

    % Score - larger, above bars
    \node[font=\scriptsize\bfseries] at (\i*2+0.9, 3.8) {\score};
  }

  % Y-axis label and ticks
  \node[rotate=90, font=\footnotesize] at (-0.8, 1.75) {Activation};
  \draw[->, thick] (-0.3, 0) -- (-0.3, 3.5);
  \node[font=\scriptsize, anchor=east] at (-0.35, 0) {0};
  \node[font=\scriptsize, anchor=east] at (-0.35, 3.5) {1};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.75]
  % Heatmap title - larger
  \node[font=\large\bfseries] at (7.5, 8.3) {Layer-Token Activation Heatmap (Backdoor Probability)};

  % Color scale - larger labels
  \foreach \i in {0,...,10} {
    \pgfmathsetmacro{\intensity}{\i*10}
    \fill[warningred!\intensity!white] (16, {7-\i*0.6}) rectangle (16.6, {7.6-\i*0.6});
  }
  \node[font=\scriptsize, anchor=west] at (16.8, 7.3) {1.0};
  \node[font=\scriptsize, anchor=west] at (16.8, 4.3) {0.5};
  \node[font=\scriptsize, anchor=west] at (16.8, 1.3) {0.0};

  % Token labels (x-axis) - improved readability
  ...
  % Layer labels (y-axis) - improved readability
  ...
  % Annotation: highlight peak - enhanced visibility with arrow
  \draw[successgreen!70!black, line width=2pt, dashed] (4.5, 4) rectangle (6, 5);
  \node[font=\footnotesize\bfseries, successgreen!40!black, fill=white, fill opacity=0.9, text opacity=1, inner sep=3pt, rounded corners=2pt] (peaklabel) at (9, 6.5) {Peak: ``2025'' at L8};
  \draw[->, successgreen!60!black, line width=1.5pt] (peaklabel.south west) -- (5.8, 5.1);
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=0.8]
  % Title - corrected head number
  \node[font=\large\bfseries] at (5, 6.8) {Attention to Trigger Tokens (Layer 14, Head 3)};
