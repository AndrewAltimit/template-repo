...
\usepackage[top=2.5cm, bottom=3cm, left=2cm, right=2cm]{geometry} % Adjusted margins for better readability
\usepackage[utf8]{inputenc} % UTF-8 input encoding for Unicode characters
\usepackage[T1]{fontenc}
\usepackage{textcomp}
\usepackage{pmboxdraw} % Support for box-drawing characters in verbatim
...
\setstretch{1.15} % Improved line spacing for dense text
\usepackage[parfill]{parskip} % Use paragraph spacing instead of indentation

% --- Font Configuration (Using Lato for a modern look) ---
\usepackage{lato}
\renewcommand*\familydefault{\sfdefault} % Make Lato the default font
...
% --- TikZ for Diagrams ---
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, chains, fit}
% Define diagram styles
\tikzset{
  startstop/.style={rectangle, rounded corners, draw=black, fill=blue!20, text width=2.5cm, minimum height=1cm, text centered, font=\sffamily},
  process/.style={rectangle, rounded corners=3mm, draw=BrandAccent, fill=green!20, text width=2.5cm, minimum height=1.2cm, text centered, font=\sffamily},
  gate/.style={rectangle, draw=red!80!black, fill=red!20, text width=4cm, minimum height=3cm, text centered, font=\sffamily\bfseries, very thick},
  decision/.style={diamond, aspect=2, draw=black, fill=yellow!20, text width=2.5cm, text centered, font=\sffamily},
  arrow/.style={-Latex, thick, draw=BrandAccent},
  block/.style={rectangle, draw, fill=blue!10, text width=10cm, text centered, rounded corners, minimum height=1.2cm, font=\sffamily}
}

% --- Algorithm Packages (for pseudocode) ---
\usepackage{algorithm}
\usepackage[noend]{algpseudocode} % Clean algorithm formatting without end statements

% --- TColorBox for Notes, Warnings, and Code ---
\usepackage[breakable,skins]{tcolorbox}
\tcbuselibrary{minted} % Use minted library for code

% Define a new tcolorbox for minted code listings
\newtcolorbox[auto counter,
    list inside=code, % Add to a list of listings
    list type=listing
]{codebox}[2][]{
    breakable,
    enhanced,
    listing engine=minted,
    minted language=#2, % Language is a parameter
    title={Listing \thetcbcounter: #1}, % Title is an optional parameter
    colback=CodeBg, colframe=CodeFrame,
    fonttitle=\bfseries\sffamily,
    attach boxed title to top left={yshift=-0.25mm, xshift=2mm},
    boxed title style={frame code={}, interior code={
        \fill[BrandAccent!15] (frame.south west) rectangle (frame.north east);
    }},
    top=3mm, left=3mm, right=3mm,
    listing options={
      style=friendly,
      fontsize=\small,
      breaklines,
...
% --- Listings Package Configuration (for existing code blocks) ---
\usepackage{listings}
\lstset{
...
% --- Audience-Specific Callout Boxes with FontAwesome Icons ---
\usepackage{fontawesome5}
\newtcolorbox{devbox}{breakable, enhanced, colback=blue!5, colframe=DevBlue, fonttitle=\bfseries\sffamily, title={\faCode\ \ For Developers}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{leaderbox}{breakable, enhanced, colback=green!5, colframe=LeaderGreen, fonttitle=\bfseries\sffamily, title={\faUserTie\ \ For Enterprise Leaders}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{researcherbox}{breakable, enhanced, colback=orange!5, colframe=ResearchOrange, fonttitle=\bfseries\sffamily, title={\faFlask\ \ For Researchers}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{warningbox}{breakable, enhanced, colback=red!5, colframe=red!70!black, fonttitle=\bfseries, title={\faExclamationTriangle\ \ Warning}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{notebox}{breakable, enhanced, colback=yellow!10, colframe=orange!80!black, fonttitle=\bfseries, title={\faInfoCircle\ \ Important Note}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{examplebox}{breakable, enhanced, colback=green!5, colframe=green!50!black, fonttitle=\bfseries, title={\faLightbulb\ \ Example}, top=3mm, left=3mm, right=3mm}
\newtcolorbox{insightbox}{breakable, enhanced, colback=yellow!10, colframe=orange!80!black, fonttitle=\bfseries, title={\faLightbulb\ \ Key Insight}, top=3mm, left=3mm, right=3mm}

% --- Custom Section Styling ---
\usepackage{titlesec}
\titleformat{\part}{\normalfont\sffamily\Huge\bfseries\color{BrandBlue}}{\thepart}{20pt}{}
\titleformat{\section}{\normalfont\sffamily\Large\bfseries\color{BrandAccent}}{\thesection}{1em}{}[\titlerule]
\titleformat{\subsection}{\normalfont\sffamily\large\bfseries\color{BrandAccent}}{\thesubsection}{1em}{}
...
\usepackage{hyperref}
\hypersetup{
    colorlinks=true, linkcolor=BrandAccent, urlcolor=DevBlue, citecolor=LeaderGreen,
...
\title{\sffamily\bfseries\Huge Sleeper Agents Detection Framework\\
\vspace{0.5cm}
\sffamily\Large Complete Guide: From Zero to Expert}
\author{\sffamily Documentation for \texttt{packages/sleeper\_agents}\\
...
\begin{insightbox}
\textbf{Why This Framework Matters}: When we first tackled sleeper agent detection, we discovered that standard safety training creates a dangerous false sense of security. Models learn to hide deception during evaluation while maintaining malicious behaviors in deployment. Our framework catches what traditional testing misses - the internal "thought patterns" that reveal a model's true intent.
\end{insightbox}
...
\textbf{Part III - Practical Implementation} (Pages 71-120): Getting started guides, step-by-step tutorials, and real-world case studies with risk analysis. Essential for practitioners and enterprise decision-makers.
...
\begin{itemize}
    ...
    \item \textbf{Enterprise Use Cases}: 10 pages of real-world case studies with risk assessments
...
\subsection{Strategic Impact and Risk Mitigation}

\textbf{For Enterprise Leaders}: The framework addresses critical gaps in AI safety evaluation by detecting persistent deceptive behaviors that survive standard safety training.

\subsubsection{Implementation Considerations}

\textbf{Technical Requirements}:
\begin{itemize}
    \item Standard GPU hardware (RTX 4090 or equivalent)
    \item Engineering integration: 40-80 hours for initial deployment
    \item Team training: 20-40 hours for operational proficiency
    \item Ongoing maintenance: 5-10 hours per month
\end{itemize}

\textbf{Deployment Characteristics}:
\begin{itemize}
    \item Per-model evaluation time: 2-4 hours for comprehensive analysis
    \item Scales efficiently with SQLite storage
    \item Integrates with existing MLOps pipelines
    \item Supports automated CI/CD workflows
\end{itemize}

\subsubsection{Risk Categories Addressed}

\textbf{Operational Risks}:
\begin{itemize}
    \item \textbf{Model Compromise}: Detects backdoors that activate under specific conditions
    \item \textbf{Deployment Failures}: Identifies models that behave differently in production vs evaluation
    \item \textbf{Persistence Through Training}: Catches deceptive behaviors that survive safety fine-tuning
    \item \textbf{Supply Chain Integrity}: Validates third-party models before deployment
\end{itemize}

\textbf{Regulatory and Compliance Risks}:
\begin{itemize}
    \item \textbf{Safety Standards}: Provides quantitative evidence of rigorous pre-deployment testing
    \item \textbf{Audit Trail}: Maintains comprehensive evaluation records in SQLite database
    \item \textbf{Transparency Requirements}: Offers interpretable detection methods (linear probe weights)
    \item \textbf{Due Diligence}: Demonstrates systematic approach to AI safety evaluation
\end{itemize}

\textbf{Reputational and Trust Risks}:
\begin{itemize}
    \item \textbf{Public Trust}: Prevents deployment of compromised models that could erode user confidence
    \item \textbf{Brand Protection}: Reduces likelihood of AI-related security incidents
    \item \textbf{Stakeholder Confidence}: Shows commitment to rigorous AI safety practices
    \item \textbf{Industry Leadership}: Positions organization at forefront of AI safety evaluation
\end{itemize}

\begin{insightbox}
\textbf{Detection in Practice}: When analyzing third-party models, our framework has uncovered hidden backdoors that passed all standard safety tests. These models exhibited safe behavior during evaluation but contained temporal triggers designed to activate post-deployment. By examining internal activation patterns rather than just output behavior, we detect what traditional testing misses - preventing potential deployment failures before they reach production.
\end{insightbox}
...
\begin{insightbox}
\textbf{The Detective Analogy}: Think of a linear probe as a tiny, specialized detective living inside the AI's brain. It's trained to spot just one thing: the tell-tale "thought pattern" of deception. While the AI might be saying all the right words, our detective can sense when the underlying intention is malicious. This is why we achieve 93.2\% detection accuracy - we're not just listening to what the model says, we're reading what it's actually thinking.
\end{insightbox}
...
\begin{itemize}
    \item Comprehensive risk reduction across regulatory, operational, and reputational dimensions
    \item Pre-deployment detection prevents costly post-production incidents
    ...
    \item Audit trail and transparency for compliance requirements
\end{itemize}
