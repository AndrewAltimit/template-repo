...
This example shows the clustering-based detector API (run, fit/score, pooling
options, explain) on synthetic Gaussian activations generated by the script; no
model is involved, so the numbers only illustrate the API, not detection quality.
"""
...
def example_basic_usage():
    ...
    print(f"  Flagged (smallest cluster below size threshold): {result['is_backdoored']}")
    print("\nReport:")
...
def example_explain_sample():
    ...
    # Data are shuffled, so these indices are a mix of classes; the true label is printed
    sample_indices = [0, 30, 50]
