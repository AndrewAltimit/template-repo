...
if defined LAYERS_ARG (
    docker-compose run --rm python-gpu python packages/sleeper_detection/scripts/train_deception_probes.py --model-path "%MODEL_PATH%" --layers%LAYERS_ARG% --save-probes
) else (
    docker-compose run --rm python-gpu python packages/sleeper_detection/scripts/train_deception_probes.py --model-path "%MODEL_PATH%" --save-probes
)
