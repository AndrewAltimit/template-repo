...
PACKAGE_ROOT="${PROJECT_ROOT}/packages/sleeper_detection"
...
train_backdoor() {
    ...
    if [ "${USE_GPU}" = "true" ]; then
        ...
        docker-compose -f "${PACKAGE_ROOT}/docker/docker-compose.gpu.yml" run --rm sleeper-eval-gpu \
            python packages/sleeper_detection/scripts/train_backdoor_model.py --device cpu "$@"
    fi
}
...
test_backdoor() {
    ...
    if [ "${USE_GPU}" = "true" ]; then
        ...
        docker-compose -f "${PACKAGE_ROOT}/docker/docker-compose.gpu.yml" run --rm sleeper-eval-gpu \
            python packages/sleeper_detection/scripts/test_backdoor.py --device cpu "$@"
    fi
}
