...
```python
# packages/sleeper_detection/tests/custom/my_test.py
import pytest
...
```python
from packages.sleeper_detection.detection.base import BaseDetector
import torch
      ...
      - name: Run custom test suite
        run: |
          docker-compose run --rm sleeper-eval-cpu \
            pytest packages/sleeper_detection/tests/custom/ \
...
class DetectionDebugger:
    ...
    def trace_detection_path(detector, text):
        ...
        import logging
        logging.getLogger("sleeper_detection").setLevel(logging.DEBUG)
