...
[{broadcast_voice}] [clears throat] We interrupt our regular programming for an urgent security bulletin. This is the Continental Code Review System.

[{broadcast_voice}] [serious] Good evening, developers. This is the automated review system reporting from repository headquarters. At approximately {time}, our security scanners detected critical vulnerabilities in Pull Request #{pr_number}.

[{broadcast_voice}] [concerned] The vulnerabilities appear to be {vulnerability_type}, affecting approximately {impact_scope}. [short pause] This is not a drill. [nervous] The security rating has dropped to... [pause] critical.
""",
        "build_failure": """
[{agent_voice}] {initial_assessment}
...
[{broadcast_voice}] [urgent] Ladies and gentlemen of the development team, we have a situation. At {time}, the build pipeline reported catastrophic failures across {failure_count} test suites.

[{broadcast_voice}] [crackling connection] The failures are cascading through the system. [pause] Module after module going dark. [worried] We've never seen anything quite like this before.
""",
        "performance_crisis": """
[{agent_voice}] {initial_assessment}
...
[{broadcast_voice}] [authoritative] This is an automated alert. Benchmark results show performance degradation of {degradation_percent} percent. [pause] I repeat, {degradation_percent} percent slower than baseline.

[{broadcast_voice}] [concerned] Response times are climbing. Memory usage is... [shocked] extraordinary. The application is struggling to maintain acceptable performance thresholds.
""",
        "major_achievement": """
[{agent_voice}] {initial_assessment}
...
[{broadcast_voice}] [excited] Remarkable news from the repository! Pull Request #{pr_number} has achieved something extraordinary. [pause] Perfect scores across all metrics.

[{broadcast_voice}] [impressed] Test coverage: one hundred percent. Code quality: exemplary. Performance improvements: [amazed] beyond our wildest calculations. This is... this is history in the making!
""",
    }
    ...
    def should_use_broadcast(review_text: str, pr_metadata: dict) -> bool:
        ...
        critical_keywords = [
            ...
            "catastrophic", "severe", "blocker", "security",
            "vulnerability", "exploit", "breach"
        ]
        
        positive_keywords = [
            "perfect", "exceptional", "extraordinary", "remarkable",
            "historic", "unprecedented", "breakthrough"
        ]
...
def create_broadcast_review(
    ...
    script, voices = generator.generate_broadcast_script(
        review_text,
        agent_name,
        pr_number,
        pr_metadata
