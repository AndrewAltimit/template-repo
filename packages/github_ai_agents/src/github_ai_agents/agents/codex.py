...
class CodexAgent(CLIAgent):
    ...
    def is_available(self) -> bool:
        """Check if Codex is available via Docker (preferred) or locally."""
            ...
            if repo_root:
                ...
                if compose_file.exists():
                    ...
                    if result.returncode == 0 and "openrouter-agents" in result.stdout:
                        ...
                        logger.info("Codex available via Docker container (preferred)")
                        return True
        ...
        if shutil.which(self.executable):
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info("Codex found locally (Docker not available)")
                    return True
            except Exception:
                pass

        self._available = False
        logger.warning("Codex not available via Docker or locally")
        return False
    ...
    def _build_command(self, prompt: str) -> List[str]:
        ...
        if self._use_docker:
            ...
            cmd.extend(["openrouter-agents", "codex"])
            cmd.extend(flags)
            cmd.append(prompt)
