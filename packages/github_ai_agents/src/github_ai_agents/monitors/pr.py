...
"""
    ...
    def _apply_review_fixes(self, pr: Dict, agent_name: str, implementation: str, branch_name: str, comment_id: str):
        ...
        try:
            ...
            # For now, we'll write the implementation to a file if it contains code
            if "```" in implementation:
                # Extract code blocks and file paths from the implementation
                logger.info("Applying code changes from agent response")
                # TODO: Implement proper code extraction and file modification
                # For now, just log that we would apply changes
                logger.info(f"Would apply code changes: {len(implementation)} characters")
    ...
    def _has_responded_to_comment(self, pr_number: int, comment_id: str) -> bool:
        ...
        if output:
            try:
                ...
                # Look for agent comments that reference this comment ID
                for comment in data.get("comments", []):
                    body = comment.get("body", "")
                    # Check if this is an agent comment responding to the specific comment
                    if self.agent_tag in body and f"comment {comment_id}" in body:
                        return True
                    # Also check for comments posted after this review that indicate work was done
                    if self.agent_tag in body and any(
                        phrase in body
                        for phrase in [
                            "I'm working on addressing this review feedback",
                            "I've successfully addressed the review feedback",
                            "I've analyzed the review feedback",
                            "I attempted to address the review feedback",
                        ]
                    ):
                        # Check if comment was created after the review comment
                        # This is a simplified check - in production, you'd compare timestamps
                        return True
            except json.JSONDecodeError:
                pass
