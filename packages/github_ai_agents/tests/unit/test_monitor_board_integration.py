...
class TestIssueMonitorBoardIntegration:
    ...
    async def test_claim_work_on_implementation(self, issue_monitor, mock_board_manager):
        ...
        with (
            ...
            patch("github_ai_agents.monitors.issue.run_gh_command_async"),
            patch("github_ai_agents.monitors.issue.run_git_command_async"),
            patch("github_ai_agents.code_parser.CodeParser.extract_and_apply", return_value=([], {})),
        ):
    ...
    async def test_release_work_on_pr_creation(self, issue_monitor, mock_board_manager):
        ...
        with (
            patch("github_ai_agents.monitors.issue.run_git_command_async") as mock_git,
            patch("github_ai_agents.monitors.issue.run_gh_command_async") as mock_gh,
            patch("github_ai_agents.code_parser.CodeParser.extract_and_apply", return_value=([], {"test.py": "created"})),
        ):
