...
\node[agent, below=2.2cm of createpr] (council) {AI Council\\[-0.1em]{\scriptsize Reviews}};
\node[agent, below=2.2cm of implement] (autofix) {Review Fix\\[-0.1em]{\scriptsize Agent Work}};
...
\node[agent, below=1.2cm of ci, minimum width=1.6cm] (cifix) {CI Fix\\[-0.1em]{\scriptsize Agent Work}};
...
% === FEEDBACK LOOPS (on background so kanban labels appear on top) ===
\begin{scope}[on background layer]
...
% Loop labels
\node[above=0.65cm of council, font=\scriptsize\color{humangate!70}\itshape, xshift=-2.8cm] {changes requested};
...
{\small \texttt{.agents.yaml} defines \texttt{agent\_admins} (approvers) and \texttt{trusted\_sources} (reviewers).}
...
{\small
Agents post reviews\\[0.2em]
$\rightarrow$ Agent auto-fixes issues\\[0.2em]
...
\begin{itemize}[leftmargin=1.2em, itemsep=0.2em]
  \item Max 5 iterations per agent type
  \item Admins can \texttt{[CONTINUE]} to extend limit
  \item Only \texttt{agent\_admins} can trigger agents
  \item Trust prioritized via \texttt{trusted\_sources}
\end{itemize}
