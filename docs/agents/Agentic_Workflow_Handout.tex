...
% === FEEDBACK LOOPS ===
% Agent auto-fix loops
\draw[looparrow] (autofix.north east) .. controls ++(0.5,0.6) and ++(-0.5,0.6) .. (council.north west);
\draw[looparrow] (autofix.north west) .. controls ++(-0.5,0.6) and ++(0.5,0.6) .. (ci.north east);

% Human review can request changes -> back to council
\draw[looparrow, humangate!70] (humanrev.south) .. controls ++(0,-0.5) and ++(0,-0.8) .. ++(4,0) .. controls ++(0,0.5) and ++(0,-0.5) .. (council.south);

% Loop labels
\node[above=0.5cm of autofix, font=\scriptsize\color{reviewloop}\itshape] {auto-fix loops};
\node[below=0.6cm of ci, font=\scriptsize\color{humangate!70}\itshape] {changes requested};
