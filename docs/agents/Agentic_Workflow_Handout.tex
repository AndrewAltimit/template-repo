...
\node[agent] (issue) {Issue\\[-0.1em]{\scriptsize Refinement}};

% Mixed human+agent node for board
\node[stage, right=of issue, draw=agentwork, fill=agentwork!8,
      path picture={
        \fill[humangate!25] (path picture bounding box.north west) rectangle
              ([xshift=0.5cm]path picture bounding box.south west);
      }] (triage) {Add to\\[-0.1em]{\scriptsize Board}};

\node[human, right=of triage] (approve) {Approve\\[-0.1em]{\scriptsize [Approved]}};
...
% Mixed label for triage
\node[above=0.15cm of triage, font=\tiny\color{darktext!60}] {human + agent};
...
\node[human, below=2.2cm of triage] (humanrev) {Review\\[-0.1em]{\scriptsize Code}};
\node[human, below=2.2cm of issue] (merge) {Merge\\[-0.1em]{\scriptsize PR}};
...
% Gate labels row 2
\node[below=0.15cm of humanrev, font=\small\bfseries\color{humangate}] {GATE 2};
\node[below=0.15cm of merge, font=\small\bfseries\color{humangate}] {GATE 3};

% Kanban above row 2
\node[kanban, above=0.3cm of council] {In Progress};
\node[kanban, above=0.3cm of autofix] {In Progress};
\node[kanban, above=0.3cm of ci] {In Progress};
\node[kanban, above=0.3cm of humanrev] {Review};
\node[kanban, above=0.3cm of merge] {Done};

% === FEEDBACK LOOPS ===
% Agent auto-fix loops (straight lines with right angles)
\draw[looparrow] (autofix.north) -- ++(0,0.4) -| (council.north);
\draw[looparrow] (autofix.north) -- ++(0,0.4) -| (ci.north);

% Human review can request changes -> back to council (straight horizontal line below)
\draw[looparrow, humangate!70] (humanrev.south) -- ++(0,-0.5) -| (council.south);

% Loop labels
\node[above=0.35cm of autofix, font=\scriptsize\color{reviewloop}\itshape] {auto-fix loops};
\node[below=0.5cm of ci, font=\scriptsize\color{humangate!70}\itshape, xshift=1cm] {changes requested};
