...
% Agent auto-fix loops (straight lines with right angles, lower than green)
\draw[looparrow] (autofix.north) -- ++(0,0.4) -| (council.north west);
\draw[looparrow] (autofix.north) -- ++(0,0.4) -| (ci.north east);

% CI failure loop (down to CI Fix, back up to CI)
\draw[looparrow, agentwork!70] (ci.south) -- (cifix.north);
\draw[looparrow, agentwork!70] (cifix.west) -| ([xshift=-0.4cm]ci.south west) -- (ci.south west);
