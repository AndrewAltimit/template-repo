...
\begin{tikzpicture}[font=\sffamily\small,
    ...
    \node (esp) [box, fill=safetyamber!15, minimum width=2.6cm, minimum height=1.1cm]
      at (4.5,0) {\textbf{ESP32-S3}\\{\tiny Real-time control}};

    % Camera (above Pi)
    \node (cam) [box, fill=visionpurple!15] at (-0.3,1.8) {Pi Camera v3};

    % Network (above between Pi and ESP)
    \node (net) [box, fill=lightteal!15] at (2.5,1.8) {WiFi/Ethernet};

    % ESP32 peripherals (right column, stacked)
    \node (ds18) [box, fill=thermalred!15] at (8.5,2.0) {DS18B20 Probes};
    \node (peltier) [box, fill=thermalred!15] at (8.5,0.9) {H-Bridge + Peltier};
    \node (stepper) [box, fill=fluidblue!15] at (8.5,0) {TMC2209 Drivers};
    \node (pump) [box, fill=fluidblue!15] at (8.5,-1.0) {Peristaltic Pumps};

    % NEMA motors (far right)
    \node (nema) [box, fill=softslate!15] at (11.8,0) {NEMA 17};

    % Pi peripherals (below Pi)
    \node (dht) [box, fill=softslate!15] at (-2.2,-1.8) {DHT22};
    \node (led) [box, fill=instrumentgold!15] at (2.2,-1.8) {LED Ring};
    \node (estop) [box, fill=safetyamber!25] at (0,-3.2) {\textbf{E-Stop}};

    % Connections -- Pi
    \draw[conn] (pi) -- (esp) node[lbl, above] {UART};
    \draw[conn] (pi) -- (cam) node[lbl, left] {CSI};
    \draw[conn] (pi) -- (net) node[lbl, above] {TCP/IP};
    \draw[conn] (pi) -- (dht) node[lbl, above] {GPIO};
    \draw[conn] (pi) -- (led) node[lbl, above] {PWM};
    \draw[conn, safetyamber, line width=1pt] (pi) -- (estop) node[lbl, right] {HW IRQ};
...
\begin{tikzpicture}[font=\sffamily\scriptsize,
    ...
    \draw[arr, dashed] (optimize) -| ([xshift=-0.5cm]media.south) -- (media.south);
...
\begin{tikzpicture}[font=\sffamily\small]
    ...
    \node (l6) [safetybar, fill=safetygreen!15] at (0,5.0)
      {\textbf{\color{safetygreen}Layer 6: Audit} --- Immutable append-only logging};
    \node (l5) [safetybar, fill=fluidblue!12] at (0,4.0)
      {\textbf{\color{fluidblue}Layer 5: MCP} --- Input validation + rate limiting};
    ...
    \node (l3) [safetybar, fill=safetyamber!12] at (0,2.0)
      {\textbf{\color{safetyamber}Layer 3: HAL} --- Volume/temperature bounds checking};
