...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[instdark, font=\small\bfseries\sffamily] at ([yshift=-1.85cm]current page.north) {PROJECTION REPORT \textbar{} ETRA-2026-PTR-001 \textbar{} FEBRUARY 2026};
...
\subsection{What We've Observed Through Early 2026}
...
\begin{itemize}
  ...
  \item Multi-agent orchestration frameworks maturing rapidly (MCP ecosystem, OpenAI Agents SDK, Claude Code agent patterns) enabling coordinated agent operations [O] [\textbf{High confidence}---public product launches and open-source repositories]
  \item Significant jailbreaking and prompt injection techniques circulating widely [O] [\textbf{High confidence}---public security research, bug bounty programs]
  \item Open-weight models approaching frontier capabilities on many benchmarks, with lag compressed to 6--12 months on structured tasks [E] [\textbf{Medium-High confidence}---benchmark comparisons show accelerating convergence, though frontier reasoning retains larger gaps]
  ...
  \item Intelligence community workforce contraction (ODNI $>$40\% cuts planned, CIA $\sim$1,200 positions, NSA reductions) compounding detection capacity challenges [O] [\textbf{High confidence}---public reporting, congressional testimony]
\end{itemize}
...
Once a capability exists at the frontier, it proliferates to open-source and less-restricted systems within 6--18 months (compressed from the 12--24 month estimate in v1.1, reflecting observed acceleration in open-weight model releases through early 2026). This means:
...
International frameworks must evolve beyond intent-based liability toward outcome-based or capability-based accountability. If you deploy an autonomous agent with the capability to target political figures, you bear responsibility for the agent's actions regardless of stated objectives. The Delegation Defense compounds the attribution void (Section 12.6): even when an attack \textit{can} be traced to state infrastructure, the defense creates a second layer of deniability.
\end{criticalbox}
...
\textbf{Note}: This table covers the four primary targeting vectors. Epistemic Contamination (Section 8.2) and Spearphishing 2.0 (Section 7.5) are cross-cutting enablers that amplify effectiveness across all four vectors. See the Risk Prioritization Matrix (Appendix A) for the complete threat landscape.
...
\begin{infobox}[Observed Aftermath (as of February 2026)]
\begin{itemize}
  \item Multiple Latin American and Global South leaders have visibly reduced international travel or restricted movements to ``safe'' jurisdictions [O]
  \item Legal scholars and state actors have publicly disputed previously settled sovereign immunity questions [O]
  \item Risk assessment firms have adjusted political risk ratings for leader travel [E]
  \item Diplomatic tensions remain elevated, with several states citing the precedent in their own territorial disputes [O]
\end{itemize}
\end{infobox}
