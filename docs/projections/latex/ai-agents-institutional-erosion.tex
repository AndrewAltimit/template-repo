...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[slatedark, font=\small\bfseries\sffamily] at ([yshift=-1.85cm]current page.north) {PROJECTION REPORT \textbar{} ETRA-2026-IC-001 \textbar{} JULY 2026};
...
Model capabilities and policy developments described in this document reflect publicly available systems and published assessments as of \textbf{early July 2026}. Where a named model or evaluation is cited, treat it as an illustrative data point on a trend, not a fixed endpoint.
\end{infobox}

\vspace{0.2cm}

\begin{infobox}[Changes from v2.0 (Mid-2026 Refresh)]
Currency refresh to July 2026 (Fable 5 / Mythos 5 frontier, accelerated METR doubling, enacted Digital Omnibus deferrals, U.S. executive-order sequence, June 2026 workforce developments); methodology provenance corrected to synthesis of published assessments; deepfake data fixed and the unsupported ``90 percent synthetic content'' figure removed; new sections (IC AI adoption as attack surface, agent deception testbeds, two additional counterarguments, expanded budget and accountability analysis); Claims Register, Glossary, and Revision History appendices; recommendation traceability table and risk-matrix rubric; em-dash constructions removed; numeric self-references replaced with named references. See Appendix C for full history.
\end{infobox}
...
\begin{defbox}[Five Load-Bearing Assumptions]
\textit{The section ``What Would Change This Assessment'' lists the developments that would invalidate each of these.}
\begin{enumerate}
  \item \textbf{[E] The capability floor has risen permanently.} Low-cost access to frontier models can automate major components of tradecraft (research, targeting, persona drafting, multilingual engagement), reducing manpower barriers even when operational constraints remain.
  \item \textbf{[E/S] Collection without verification is now a liability.} Decoy generation is now orders of magnitude cheaper than triage; where triage is manual, decoys can plausibly outnumber authentic signals ($\sim$3--30$\times$ as an illustrative range; see the Collection-to-Verification Pivot). Traditional signal-to-noise filters become unreliable.
  \item \textbf{[E] Attribution of intent is structurally harder.} The ``Delegation Defense'' (blaming autonomous agent behavior) provides plausible deniability for state actors using AI agents.
  \item \textbf{[E] Institutional speed cannot match adversary iteration.} Adversaries iterate at software speed; IC adoption is constrained by procurement, authorities, and assurance requirements.
  \item \textbf{[O] The verification workforce is contracting.} Confirmed IC staffing reductions: NSA met 2,000-person reduction (Dec 2025); ODNI cut toward $\sim$1,300 under ``ODNI 2.0,'' dissolving the Foreign Malign Influence Center, with several hundred further cuts sought under the acting DNI (June 2026); CIA shrinking $\sim$1,200 positions; $\sim$8\% annual DOD budget cuts affect all military intelligence.
\end{enumerate}
\end{defbox}
...
``Agents aren't this capable yet'' & Capabilities described are current (July 2026); METR doubling has accelerated to $\sim$4 months, and frontier labs now ship models they assess as capable of uplifting well-resourced threat actors (Fable 5 / Mythos 5, June 2026) \\
\bottomrule
\end{tabular}
\end{center}
\end{warnbox}
...
\textbf{Current IC posture (as of early July 2026) [O/E]}:
\begin{itemize}[nosep]
  \item AI integration initiatives underway; IC-wide AI roadmap targets ``AI enabling services at scale'' for FY2026-2029
  \item Significant variation in adoption maturity across the 18-element community
  ...
  \item Confirmed workforce contraction (NSA -2,000; ODNI reduced by an announced $\sim$40\%, from $\sim$2,000 toward $\sim$1,300; CIA -1,200) concurrent with rising verification demands
  \item FMIC dissolved; counterproliferation and cyber threat integration centers restructured
  \item June 2026: further ODNI reductions of several hundred positions sought under the acting DNI, over formal congressional objections (Government Executive, June 2026)
\end{itemize}
...
\begin{itemize}
  ...
  \item We do not claim AI agents make traditional intelligence collection obsolete; human judgment remains essential, and the shift is that verification becomes the bottleneck
  ...
  \item We do not claim catastrophic failure is inevitable; avoiding it requires a deliberate pivot
\end{itemize}
\end{infobox}
...
AI Agents and Political Targeting & ETRA-2026-PTR-001 (v2.1) & Validates IC erosion dynamics through Verification Pivot and Algorithmic Capture; its Insider Threat vector parallels the IC AI-adoption attack surface here \\
AI Agents and WMD Proliferation & ETRA-2026-WMD-001 (v2.1) & Nano-smurfing for dual-use materials; ``conspiracy footprint shrinks''; attribution void \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{Current capability assessment} of AI agent systems as publicly documented through early July 2026
  \item \textbf{Institutional analysis} of IC structure, incentives, and historical adaptation patterns
  \item \textbf{Open-source reporting} on adversary AI adoption and doctrine
  \item \textbf{Synthesis of published expert analysis} across intelligence studies, AI safety, and national security law
  \item \textbf{Published red-team and evaluation results} (frontier-lab system cards, METR task-horizon research, and public red-teaming literature)
\end{itemize}

This is independent, single-author research: it involves no first-party expert consultation, no access to classified material, and no red-team exercises conducted by or for the author. We deliberately avoid classified information, specific operational details that could enable harm, named targeting scenarios, and technical implementation details for adversarial applications.
...
\midrule
Agent task-horizon doubling & $\sim$7 months (2019--2025 data); $\sim$4 months (2024-onward data) & METR (Mar 2025; Jan 2026 update) \\
Current autonomous task horizon & Frontier 50\%-time horizons exceed the $\sim$16-hour reliable measurement range of current task suites & METR (May 2026); up from $\sim$1-hour (early 2025) \\
Enterprise AI agent penetration & 40\% of apps by end 2026 & Gartner (up from <5\% in 2025) \\
Multi-agent system interest & $\sim$14x inquiry surge, Q1 2024 to Q2 2025 & Gartner inquiry data \\
AI-generated code share & Majority of new code at some frontier labs & Company statements (2026); see Claims Register \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Trajectory Milestones [O]}: On February 5, 2026, OpenAI launched GPT-5.3-Codex and Anthropic released Opus 4.6 with autonomous agent teams on the same day; the Agentic AI Foundation (AAIF) was jointly launched by OpenAI, Anthropic, and Google. In June 2026, Anthropic released Claude Fable 5 / Claude Mythos 5, one model in two configurations: Fable 5 for general use with classifier safeguards blocking high-risk domains, and Mythos 5 with safeguards lifted for a small set of vetted partners. The system card assesses the unsafeguarded configuration as able to ``significantly uplift well-resourced threat actors.'' Frontier labs now treat threat-actor uplift as a live property of shipping models, and the safeguard layer, not base capability, is what separates the general-purpose product from the uplift-capable one.
\end{criticalbox}
...
\textbf{The 2026 Reality}: The marginal cost of generating a plausible decoy signal is now orders of magnitude below the cost of triaging it. Where generation is cheap and triage is manual, decoys can plausibly outnumber authentic signals; we treat 3--30x as an illustrative range [S] pending measurement, highest on public-facing surfaces and lowest inside authenticated systems. The Lead Decay Rate metric is designed to replace this judgment with measurement. Traditional signal-to-noise filters become unreliable.
...
METR research (March 2025) measured AI agent task-completion horizons doubling every $\sim$7 months on 2019--2025 data; subsequent updates found the rate accelerating to roughly 4 months on 2024-onward data. By May 2026, frontier time horizons exceeded the $\sim$16-hour range that METR's current task suite can reliably measure. Even at the slower historical rate, generalist autonomous agents will handle week-long tasks within 1--2 years, while IC procurement cycles for comparable tools measure in years.
\end{keybox}
...
\begin{center}
\texttt{Verification Capacity (VC) $\sim$ Experienced verifier headcount $\times$ Cross-agency integration bandwidth $\times$ Tool reliability}
\end{center}
...
Leadership & NSA/Cyber Command head dismissal; DOGE classified system access; Senate Intel concerns & AP, NPR, Sen.\ Warner & Institutional continuity disruption; insider threat vector \\
ODNI (2026) & Several hundred further cuts sought under the acting DNI after the $\sim$40\% reduction already realized; formal congressional objections & GovExec, Reuters (June 2026) & Continued contraction of integration workforce during the contamination surge \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Partial counter-signal [O]}: After 12+ months of freezes, most IC agencies reopened hiring in February 2026, primarily administrative and legal roles. A stabilization indicator, but it does not offset the experience drain above, and the June 2026 developments cut against it.
\end{criticalbox}
...
\begin{warnbox}[Deepfake Growth and Detection Ceiling {[D/E]}]
\begin{center}
\small
...
\textbf{Metric} & \textbf{Value} \\
\midrule
Estimated online deepfakes, 2023 & $\sim$500,000 (industry aggregators) \\
Estimated online deepfakes, 2025 & $\sim$8,000,000 ($\sim$16x over two years, about 4x per year) \\
Organizations reporting a deepfake attack in prior 12 months & 62\% (n=302; Gartner survey, Sept 2025) \\
\bottomrule
\end{tabular}
\end{center}

\textit{A number we do not use}: the circulated claim that ``90 percent of online content will be AI-generated by 2026'' traces to a misreading of a 2022 Europol report and has no measurement behind it; it is excluded deliberately.

\textbf{Detection Accuracy Ceiling [E]}: Reported detection accuracies cluster in the 80--90\% range on academic benchmarks and degrade sharply out of distribution and under adversarial adaptation: well below intelligence-grade reliability. Detection tools are ``failing in the places and moments where they're needed most'' (TechPolicy.Press, 2026).
\end{warnbox}
...
Supply-chain compromise & Malicious model updates, compromised dependencies in procured AI tools & Provenance Ladder (Layer 2) \\
Behavioral backdoors & Adversary data poisoning during training creates triggered behaviors that evade pre-deployment testing & Synthetic Content Detection (sleeper-agents framework) \\
Insider-mediated access & Personnel with anomalous access configure or query AI tools outside oversight & Documented DOGE access (NPR, Apr 2025; Senate Intelligence Committee concerns) \\
\bottomrule
\end{tabular}
\end{center}
\end{warnbox}
...
Anthropic's June 2026 Fable 5 / Mythos 5 system card names \textbf{``undermining decisions within major governments''} as an assessed alignment-risk pathway (Pathway 8), noting that a model used widely inside a government could ``shape the information and ideas that reach important decision-makers,'' and that the lab has limited visibility into government deployments and therefore limited ability to rule out third-party behavioral backdoors planted via data poisoning. The system card's mitigating factors (conservative government adoption, visible reasoning traces) are exactly the factors the Verification Pivot erodes: adoption pressure rises with verification workloads, and reasoning traces help only if someone is staffed to read them.
\end{criticalbox}

\textbf{The Structural Trap [E]}: Workforce contraction pushes agencies toward AI-assisted verification; AI-assisted verification expands the Algorithmic Capture surface; a captured verification pipeline is worse than a slow one, because it converts the IC's authority into the adversary's instrument. The falsifiable test from the Definitions section applies: if an adversary can systematically shift analytic conclusions without changing ground truth, the verification layer itself is compromised.
...
\textit{Critical} = plausible failure would compromise decisions at the presidential/NSC level or disable a statutory mission. \textit{High} = plausible sustained degradation of a primary mission. \textit{Medium} = efficiency and confidence losses without mission failure. \textit{Immediate} = adaptation must begin within the current budget cycle; \textit{Near-term} = within 1--2 years; \textit{Ongoing} = continuous adaptation within existing programs. Ratings are structured analyst judgment, not model outputs. The 15 rows cover all 18 IC elements; the four service intelligence organizations are aggregated into one row.
\end{infobox}
...
\addlinespace
``Agents Aren't That Capable Yet'' & [O] METR task-horizon doubling accelerated to $\sim$4 months on recent data; frontier horizons exceed the $\sim$16-hour reliable measurement range (May 2026). The Fable 5 / Mythos 5 system card (June 2026) assesses significant threat-actor uplift for the unsafeguarded configuration. 40\% enterprise AI agent penetration by end 2026 (Gartner). Even imperfect agents create problems. \\
\addlinespace
...
\addlinespace
``Verification Was Always the Job'' & Correct about doctrine (ICD 203 has mandated source validation for two decades), wrong about economics: contamination now scales at near-zero marginal cost while verification remains labor-bound, and OSINT flips from cheap corroboration layer to expensive liability. The pivot is a change in resource allocation and institutional priority, continuous with existing tradecraft rather than a repudiation of it. \\
\addlinespace
``The Minimal-Effects Literature'' & Empirical research finds small mass-persuasion effects from disinformation; this report's claims do not depend on persuasion. Process DoS is a load phenomenon, Verification Latency a cost phenomenon, and the liar's dividend operates through doubt. The Venezuela/Maduro case mattered because fabricated content consumed verification capacity, not because it changed minds. \\
\end{longtable}
}
...
\end{itemize}
The verification pivot should include \textbf{active detection}: using controlled environments to identify, characterize, and understand adversary agent operations. This capability is specified as a recommendation under Operational Measures (Agent Deception Testbeds), including its legal and policy review gate.
\end{keybox}
...
A fourth possibility, to which we assign the residual $\sim$5--10\% [S] left by the three primary scenarios: the global information environment fragments into ``provenance islands,'' trusted zones where authentication is maintained, surrounded by ``epistemic wilderness'' where nothing can be verified.
...
\textbf{Leading indicator}: divergent and mutually unrecognized national content-authentication mandates (incompatible provenance regimes from EU content-marking rules, U.S. voluntary frameworks, and Chinese labeling requirements) would be the earliest observable sign of island formation.
\end{defbox}
...
Every measure below maps to one of the two Priority Control buckets above (A: protect leadership workflows; B: scale verification throughput) and to a primary risk from the agency risk matrix.

\begin{center}
\small
\begin{tabular}{L{4.2cm}L{0.8cm}L{2cm}L{2.2cm}L{3.2cm}}
\toprule
\textbf{Recommendation} & \textbf{Bkt} & \textbf{Tier} & \textbf{Owner} & \textbf{Primary Risk Mitigated} \\
\midrule
Verification Pipeline instrumentation & B & Bronze-Gold & ODNI & Verification Latency (all) \\
Model Provenance \& Verification Ladder & B & Bronze-Gold & NSA + CISA & Contamination; attribution gap \\
Synthetic Content Detection & B & Bronze-Gold & CISA, NGA, NSA & OSINT/GEOINT erosion \\
Bounty Agent (red team + bounty) & B & Bronze-Silver & Each agency & Process DoS; Algorithmic Capture \\
Verification Latency Baseline & B & Bronze & ODNI & Metric foundation \\
Analog Break + Hardware-Provenanced Comms & B & Silver-Gold & CIA, DIA & HUMINT degradation \\
Agent Deception Testbeds & B & Silver & FBI; CIA/NSA & Adversary agent characterization \\
Human-in-the-loop for high-stakes intel & A & Bronze & CIA, DIA & Capture of leadership products \\
AI Supply Chain Audit & A & Silver-Gold & ODNI + CTOs & IC AI adoption attack surface \\
Decision Diffusion Framework & A & Silver-Gold & NSC & Leadership targeting \\
Five Eyes verification pilot & B & Gold & State INR + ODNI & Allied-trust erosion; leapfrog \\
\bottomrule
\end{tabular}
\end{center}

\begin{infobox}[A Note on Owners]
Several recommended owners sit outside the 18-element IC: CISA is a DHS operational agency, and the NSC is a policy body. This is deliberate (verification infrastructure is broader than the IC), but those items require coordination authorities beyond DNI direction, which their timelines should account for.
\end{infobox}
...
\midrule
1. Content Credentials & High (now); CAI 6,000+ members; C2PA v2.1; Conformance Program and Trust List operational (ITL frozen Jan 2026) & Cryptographic proof for content you control; voluntary, most content unsigned; adversaries won't cooperate \\
...
\subsection{Agent Deception Testbeds (Silver)}

Controlled environments that detect and characterize adversary agent activity against IC-facing surfaces; extensions of honeypot tradecraft adapted for AI agents. The goal is intelligence collection about adversary agent capabilities, not ``trapping'' in an offensive sense.

\begin{recbox}[Testbed Components]
\begin{center}
\small
\begin{tabular}{L{3.8cm}L{5.4cm}L{3cm}}
\toprule
\textbf{Component} & \textbf{Function} & \textbf{Owner} \\
\midrule
Behavioral signature collection & Instrument decoy surfaces (synthetic tip lines, decoy documents, seeded personas) to capture how agents probe & FBI (domestic); CIA/NSA (foreign-facing) \\
Capability characterization sandbox & Observe captured agent sessions in isolation to profile model family, tooling, autonomy & NSA + Provenance Ladder (Layer 3) \\
...
\textbf{Boundary}: Active engagement or deception operations against identified operators are out of scope and require separate authorities.
\end{recbox}
...
EU AI Act (as amended by Digital Omnibus) & Omnibus approved June 29, 2026: Annex III high-risk deferred to Dec 2, 2027 (Annex I to Aug 2028); Art.\ 50 transparency applies Aug 2, 2026; watermarking Dec 2, 2026 & Enforcement deferred and untested; no extraterritorial reach for state intelligence uses \\
U.S. executive actions & EO 14110 revoked (Jan 2025); EO 14365 (Dec 2025) directs state-law preemption and a DOJ AI Litigation Task Force; June 2, 2026 EO adds cybersecurity mandates and a voluntary frontier-deployment framework & Deregulatory direction; voluntary frameworks impose no verification or provenance obligations \\
UK AI Security Institute & Operating; pre-deployment capability and robustness testing of frontier models (incl.\ Fable 5 / Mythos 5, June 2026) & Evaluation capacity, not regulation; coverage limited to cooperating labs \\
Treaty-Following AI (TFAI) & Emerging research concept & Technical, legal, political hurdles unresolved \\
\bottomrule
\end{tabular}
\end{center}

\textbf{The Governance Vacuum Persists [E]}: No binding international framework currently constrains state use of AI agents for intelligence operations as of early July 2026.
\end{infobox}

\begin{recbox}[Concrete Gold-Tier Deliverable: Five Eyes Verification-Interoperability Pilot]
\begin{itemize}[nosep]
  \item \textbf{Shared provenance standards}: common content-credential and chain-of-custody formats for exchanged intelligence (ODNI + partners)
  \item \textbf{Mutual recognition}: an authentication level assigned by one partner is accepted by the others, with defined confidence mapping (ODNI)
  \item \textbf{Joint latency benchmarking}: partners measure Verification Latency against a common test set; results shared quarterly (State INR + ODNI)
  \item \textbf{Evaluation cooperation}: extend the UK AI Security Institute model of pre-deployment frontier-model testing into shared adversary-agent characterization
\end{itemize}
...
This report recommends significant verification investment while documenting $\sim$8\% annual DOD cuts and IC-wide contraction. These are reconcilable only through reallocation: collection that cannot be verified is not merely low-value, it is negative-value, because it consumes triage capacity and carries contamination into products. Under a shrinking topline, every dollar spent expanding unverifiable collection makes the verification deficit worse. The pivot is a reweighting of the existing bill, not a new one.
\end{keybox}

\begin{infobox}[Mechanism and Cost Classes]
\textbf{Mechanism [E]}: The National Intelligence Program (NIP) and Military Intelligence Program (MIP) are the two levers. The pivot requires a named \textbf{verification throughput line item} in NIP budget justification materials (today the spend is invisible inside collection and analysis program elements), justification language tied to Verification Latency and False Clean metrics rather than collection volume, and MIP alignment so service elements are not incentivized to backfill collection cuts by cutting verification staff first.

\vspace{0.5em}
\textbf{Cost classes [S] (illustrative)}: Bronze items are staff-time and policy actions inside existing budgets; Silver items are procurement-scale (tens of millions IC-wide); Gold items are program-scale (low hundreds of millions over several years). A reallocation of even a low single-digit percentage of annual collection spend would fund every Gold item in this report; the constraint is institutional will and metric design, not topline dollars.

\vspace{0.5em}
\textbf{Why metrics lead}: Until Verification Latency and Lead Decay are measured, verification competes for budget as an unquantified virtue and loses to collection numbers every time. Once measured, the case makes itself or fails honestly.
\end{infobox}
...
\small
\begin{tabular}{L{4cm}L{4cm}L{4.5cm}}
\textbf{If Accurate} & \textbf{If Overstated} & \textbf{Status (early July 2026)} \\
\midrule
Agencies report Process DoS impact & Lead volumes stable & Partial: Venezuela/Maduro event; no public reporting of sustained agency-level Process DoS since \\
Verification Latency is concern & Not discussed & Partial: IC AI strategy discussions; no public latency metric adopted \\
Major failure from contamination & No contamination failures & Partial-to-yes: Venezuela/Maduro surge overwhelmed public verification while FMIC was dissolved; internal impact not publicly knowable \\
``Ground truth'' discussions & OSINT reliability unchanged & Partial: deepfake volume $\sim$16x 2023-2025; 62\% of surveyed organizations report attacks (Gartner) \\
Budget includes verification & Collection-centric continues & \textbf{No}: DOD cuts, continued IC reductions (June 2026), no verification line item \\
\end{tabular}
\end{center}

\textbf{Assessment}: Five months in, the evidence is mixed and directionally consistent: contamination-side indicators are partially validated while adaptation-side indicators show no movement. The strongest disconfirming observation to date is the absence of publicly reported, sustained Process DoS at any agency. Full re-evaluation at the 2027 check.
\end{defbox}
...
Verification capacity is opaque to public oversight, and several recommendations in this report build domestic-facing machinery: automated triage of public tips and FOIA requests touches First Amendment-protected activity, and a Process DoS defense misapplied to lawful mass participation is itself an institutional-erosion failure mode. Three oversight hooks follow: (1) \textbf{reportable error rates}: False Clean and false-positive rates for domestic-facing verification pipelines reported to the congressional intelligence and judiciary committees, and in aggregate publicly, as FISA statistics are; (2) \textbf{explicit mandates for existing bodies}: PCLOB and agency inspectors general given jurisdiction over synthetic-content triage systems before deployment; (3) \textbf{adversarial audit}: a civil-liberties track in the external Bounty Agent program probing for discriminatory or viewpoint-correlated misclassification. The verification pivot asks the public to trust the IC as an arbiter of authenticity; that trust is itself a verification problem, solved by instrumentation, audit, and published error rates, not assertion.
\end{infobox}
...
The register below anchors the datable [O]/[D] claims in the body to public sources. This is an independent analysis; sources are representative anchors for verification, not an exhaustive citation apparatus.

\begin{enumerate}[itemsep=2pt]
  \item Maduro capture and disinformation surge (7 major fakes in first week; 14M+ views in under 2 days; more fake than real content estimated; one detectable SynthID watermark). Jan 3, 2026 and following week. \textit{NewsGuard; NBC News; NPR; CNBC; SCMP.}
  \item FMIC dissolved; counterproliferation and cyber threat integration centers folded into Mission Integration under ``ODNI 2.0.'' Aug 20, 2025. \textit{CNN; PBS News; DNI.gov fact sheet.}
  \item ODNI reduction announced at $\sim$40\% (from $\sim$2,000 toward $\sim$1,300), \$700M+ annual savings claimed. Aug 2025. \textit{Federal News Network; PBS News.}
  \item Further ODNI cuts of several hundred positions sought under the acting DNI; congressional objections. June 2026. \textit{Government Executive; Reuters; CNN.}
  \item NSA 2,000-person civilian reduction target met. By end of 2025. \textit{Nextgov/FCW (Dec 2025); Defense One.}
  \item CIA shrinking $\sim$1,200 positions ($\sim$5\%) via attrition and reduced hiring. Reported 2025. \textit{AP News; Washington Post.}
  \item DOGE staffers gained accounts on classified DOE networks; Senate Intelligence Committee concerns. Apr 2025. \textit{NPR; warner.senate.gov.}
  \item DOD budget direction: $\sim$8\% annual cuts over five years. 2025 guidance. \textit{Defense press reporting.}
  \item METR task-horizon research: $\sim$7-month doubling (2019--2025); $\sim$4-month doubling (2024-onward); frontier horizons exceed the $\sim$16-hour reliable measurement range. Mar 2025; Jan 2026; May 2026. \textit{metr.org.}
  \item GPT-5.3-Codex and Claude Opus 4.6 launched same day; AAIF founded by OpenAI, Anthropic, Google. Feb 5, 2026. \textit{Vendor announcements; contemporaneous coverage.}
  \item Claude Fable 5 / Mythos 5 tiered release; system card assesses significant threat-actor uplift for the unsafeguarded configuration and names ``undermining decisions within major governments'' as an alignment-risk pathway; UK AISI external testing. June 9, 2026. \textit{Anthropic system card.}
  \item Voice cloning crossed the ``indistinguishable threshold.'' Dec 2025. \textit{Fortune.}
  \item CAI 6,000+ members; C2PA v2.1; Conformance Program and Trust List operational (ITL frozen Jan 1, 2026); DigiCert and SSL.com conformant CAs. Jan 2026 status. \textit{contentauthenticity.org; c2pa.org.}
  \item Deepfake volume estimates $\sim$500K (2023) to $\sim$8M (2025); Gartner survey: 62\% of organizations (n=302) experienced a deepfake attack in prior 12 months. \textit{Industry aggregators; Gartner (Sept 2025).}
  \item Detection accuracies cluster 80--90\% on benchmarks, degrading out of distribution. 2026. \textit{TechPolicy.Press; academic literature.}
  \item Pentagon: China's LLM progress has ``narrowed the performance gap.'' Dec 2025. \textit{DefenseScoop.}
  \item Russia C4ISR modernization includes LLM-based assistants; Sberbank-China AI cooperation. 2025. \textit{CSIS; VOA.}
  \item EU AI Act Digital Omnibus: agreement May 7, 2026; Parliament June 16; Council June 29. Annex III deferred to Dec 2, 2027; Annex I to Aug 2, 2028; Art.\ 50 transparency Aug 2, 2026; watermarking Dec 2, 2026. \textit{Law-firm client alerts; TechPolicy.Press.}
  \item EO 14110 revoked (Jan 23, 2025); EO 14365 (Dec 11, 2025); June 2, 2026 frontier-security EO. \textit{whitehouse.gov; legal analyses.}
  \item Gartner: 40\% of enterprise apps to embed AI agents by end 2026; $\sim$14x multi-agent inquiry surge. 2025 forecasts. \textit{Gartner.}
  \item Majority of new code AI-generated at some frontier labs. 2026 company statements; treat as self-reported.
  \item IC hiring partially reopened (administrative and legal roles). Feb 2026. \textit{Trade press.}
  \item CoE Framework Convention on AI adopted May 2024; not yet in force. \textit{coe.int.}
\end{enumerate}
...
\begin{description}[itemsep=1pt]
  ...
  \item[Analog Break] Physical-only verification (in-person meetings, physical documents, biometrics) inserted into a digital workflow to defeat synthetic personas.
  \item[Attribution-Intent Gap] The structural difficulty of establishing human intent when actions are executed by agents that may have derived methods independently.
  \item[Capability Floor] The minimum capability accessible to actors at a given resource tier; agents raise it.
  \item[Delegation Defense] A principal's disclaimer of responsibility for methods an autonomous agent derived; aliases: Plausible Deniability 2.0 (this report), Principal-Agent Defense (Economic Actors report).
  ...
  \item[False Clean] Contaminated content incorrectly marked authentic; the most dangerous verification failure mode.
  \item[GenSP] Generative Spearphishing: hyper-personalized, multi-channel recruitment or phishing approaches generated at industrial scale.
  \item[Goal Archaeology] Forensic methods for tracing an agent's goal specification back to its human principal.
  ...
  \item[Process DoS] Overwhelming an organization's investigative or analytical capacity with plausible-but-false leads, requests, or data.
...
\begin{center}
\small
\begin{tabular}{L{1.2cm}L{2.2cm}L{9cm}}
\toprule
...
2.1 & July 2026 & Mid-2026 refresh: Fable 5 / Mythos 5 frontier and accelerated METR doubling; governance table rebuilt (Digital Omnibus, U.S. EOs, UK AISI); June 2026 workforce developments; methodology provenance corrected; deepfake data fixed, ``90\% synthetic'' removed; new sections (IC AI adoption attack surface, testbeds, two counterarguments, budget and accountability expansion); Claims Register, Glossary, Revision History appendices; traceability table and rubric; em-dashes removed \\
\bottomrule
\end{tabular}
\end{center}
