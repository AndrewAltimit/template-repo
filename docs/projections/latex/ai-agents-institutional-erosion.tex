...
\begin{criticalbox}[Documented Workforce Contraction {[O]}]
...
\toprule
\textbf{Agency} & \textbf{Reported Action} & \textbf{Source} & \textbf{Verification Impact} \\
\midrule
ODNI & Planned staffing cuts >40\% by Oct 2025; prior 25\% reduction already implemented; elimination of certain units & Reuters & Reduced cross-agency integration bandwidth \\
CIA & Shrinking $\sim$1,200 positions over several years; ``hundreds already taking early retirement'' & AP News & Loss of experienced analysts and case officers \\
NSA & ``Thousands'' of positions cut (part of broader IC reduction) & AP News & Reduced SIGINT verification capacity \\
Leadership & Abrupt dismissal of NSA/Cyber Command head; litigation around personnel actions & AP News, WaPo & Institutional continuity disruption \\
\bottomrule
\end{tabular}
\end{center}
\end{criticalbox}
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \begin{scope}[shift={(0.32\paperwidth, -7.9cm)}]
    ...
    % Managed: 30-40% (height ~0.8)
    \fill[verifyteal, opacity=0.9] (-2.4, 0) rectangle (-0.8, 0.8);
    \node[white, font=\fontsize{9}{9}\selectfont\bfseries] at (-1.6, 0.4) {30-40\%};
    \node[textlight, opacity=0.8, font=\fontsize{8}{8}\selectfont] at (-1.6, -0.35) {Managed};

    % Parity: 40-50% (height ~1.2, tallest)
    \fill[stressorange, opacity=0.9] (-0.5, 0) rectangle (1.1, 1.2);
    \node[white, font=\fontsize{9}{9}\selectfont\bfseries] at (0.3, 0.6) {40-50\%};
    \node[textlight, opacity=0.8, font=\fontsize{8}{8}\selectfont] at (0.3, -0.35) {Parity};

    % Collapse: 10-20% (height ~0.5, shortest)
    \fill[burgundydeep, opacity=0.9] (1.4, 0) rectangle (3.0, 0.5);
    \node[white, font=\fontsize{9}{9}\selectfont\bfseries] at (2.2, 0.25) {10-20\%};
...
\textbf{Version}: & 1.0 (Working Draft) \\
