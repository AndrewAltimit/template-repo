...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  % Visualization: Defense-in-Depth Layers (Clean stacked bar design)
  \begin{scope}[shift={(0.5\paperwidth, -7cm)}]
    % Defense layer bars - from outer (weak) to inner (strong)
    % Each bar has: background fill, border, label on left, description on right

    % Layer 1: Transaction Monitoring (outermost, most porous)
    \fill[mintlight, opacity=0.15] (-4.5, 1.2) rectangle (4.5, 1.8);
    \draw[mintlight, opacity=0.5, line width=1pt, dashed] (-4.5, 1.2) rectangle (4.5, 1.8);
    \node[mintlight, font=\fontsize{8}{8}\selectfont\sffamily, anchor=east] at (-4.7, 1.5) {OUTER};
    \node[white, opacity=0.9, font=\fontsize{9}{9}\selectfont\sffamily] at (0, 1.5) {Transaction Monitoring};
    \node[white, opacity=0.5, font=\fontsize{7}{7}\selectfont, anchor=west] at (4.7, 1.5) {porous};

    % Layer 2: KYC/KYB Verification (middle)
    \fill[moneygreen, opacity=0.2] (-3.5, 0.4) rectangle (3.5, 1.0);
    \draw[moneygreen, opacity=0.6, line width=1.5pt] (-3.5, 0.4) rectangle (3.5, 1.0);
    \node[moneygreen, font=\fontsize{8}{8}\selectfont\sffamily, anchor=east] at (-3.7, 0.7) {MIDDLE};
    \node[white, opacity=0.9, font=\fontsize{9}{9}\selectfont\sffamily] at (0, 0.7) {Identity Verification (KYC/KYB)};
    \node[white, opacity=0.5, font=\fontsize{7}{7}\selectfont, anchor=west] at (3.7, 0.7) {moderate};

    % Layer 3: Endpoint Control (innermost, strongest)
    \fill[copperrich, opacity=0.25] (-2.5, -0.4) rectangle (2.5, 0.2);
    \draw[copperrich, opacity=0.8, line width=2pt] (-2.5, -0.4) rectangle (2.5, 0.2);
    \node[copperrich, font=\fontsize{8}{8}\selectfont\sffamily\bfseries, anchor=east] at (-2.7, -0.1) {INNER};
    \node[white, opacity=0.95, font=\fontsize{9}{9}\selectfont\sffamily\bfseries] at (0, -0.1) {Endpoint Control};
    \node[copperrich, opacity=0.8, font=\fontsize{7}{7}\selectfont, anchor=west] at (2.7, -0.1) {robust};

    % Core: Protected System
    \fill[emeralddeep] (-1.2, -1.1) rectangle (1.2, -0.6);
    \node[white, font=\fontsize{8}{8}\selectfont\bfseries] at (0, -0.85) {PROTECTED SYSTEM};

    % Threat arrows from sides
    \draw[fraudred, opacity=0.6, line width=1.5pt, -Stealth] (-6, 1.5) -- (-4.6, 1.5);
    \draw[fraudred, opacity=0.6, line width=1.5pt, -Stealth] (6, 1.5) -- (4.6, 1.5);
    \node[fraudred, opacity=0.7, font=\fontsize{7}{7}\selectfont] at (-6, 1.1) {Threats};
    \node[fraudred, opacity=0.7, font=\fontsize{7}{7}\selectfont] at (6, 1.1) {Threats};
  \end{scope}
