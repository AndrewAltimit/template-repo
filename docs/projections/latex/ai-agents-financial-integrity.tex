...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  % Legend box - centered below graph
  \node[anchor=north] at (0.5\paperwidth, -9.0cm) {
    \begin{tikzpicture}
      \fill[black, opacity=0.3, rounded corners=3pt] (-3.5, -0.4) rectangle (3.5, 0.4);
      \fill[fraudred, opacity=0.4] (-3.2, -0.15) rectangle (-2.8, 0.15);
      \node[white, opacity=0.9, font=\fontsize{9}{9}\selectfont\sffamily, anchor=west] at (-2.6, 0) {Vulnerability Window = Policy Risk};
    \end{tikzpicture}
  };
