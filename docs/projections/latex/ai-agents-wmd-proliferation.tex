...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \node[white, opacity=0.7, font=\normalsize\sffamily] at (0.5\paperwidth, -5.2cm) {Deployment Pathways and Cyber-Physical Integration};

  % Convergence arrows visualization - shifted right for label visibility
  \begin{scope}[shift={(5cm, -7.2cm)}]
    % Subtle background panel
    \fill[black, opacity=0.15, rounded corners=3pt] (-3.8, -1.8) rectangle (4, 1.8);
    % Three converging paths to center - wider spread
    \draw[->, scienceteal, opacity=0.8, line width=2.5pt] (-2.5, 1.4) -- (-0.4, 0.15);
    \draw[->, labcyan, opacity=0.8, line width=2.5pt] (-2.5, 0) -- (-0.4, 0);
    \draw[->, biogreen, opacity=0.8, line width=2.5pt] (-2.5, -1.4) -- (-0.4, -0.15);
    % Source labels - more padding from edge
    \node[white, opacity=0.95, font=\fontsize{8}{8}\selectfont\sffamily, anchor=east] at (-2.7, 1.4) {CYBER};
    \node[white, opacity=0.95, font=\fontsize{8}{8}\selectfont\sffamily, anchor=east] at (-2.7, 0) {AI};
    \node[white, opacity=0.95, font=\fontsize{8}{8}\selectfont\sffamily, anchor=east] at (-2.7, -1.4) {PHYSICAL};
    ...
    % Output arrow - longer
    \draw[->, barriergold, opacity=0.95, line width=3pt] (0.4, 0) -- (2.5, 0);
    \node[white, opacity=0.95, font=\fontsize{8}{8}\selectfont\sffamily, anchor=west] at (2.7, 0) {THREAT};
  \end{scope}

  % Legend - repositioned
  \node[white, opacity=0.5, font=\fontsize{7}{7}\selectfont, anchor=west] at (10cm, -6.2cm) {CONVERGENCE MODEL};
  \node[white, opacity=0.6, font=\fontsize{6}{6}\selectfont, anchor=west, text width=5cm] at (10cm, -6.9cm) {Multiple capability streams combining into novel threats};
