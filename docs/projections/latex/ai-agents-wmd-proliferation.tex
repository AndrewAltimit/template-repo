...
% ============================================================================
% COLOR DEFINITIONS - Academic/Research Theme (less threatening)
% ============================================================================
\definecolor{academicnavy}{HTML}{2C3E50}
\definecolor{academicblue}{HTML}{3498DB}
\definecolor{softslate}{HTML}{5D6D7E}
\definecolor{researchgreen}{HTML}{27AE60}
\definecolor{warnambber}{HTML}{D4AC0D}
\definecolor{softcoral}{HTML}{E74C3C}
\definecolor{infoteal}{HTML}{17A2B8}
...
\definecolor{biogreen}{HTML}{52BE80}
\definecolor{chemblue}{HTML}{5DADE2}
\definecolor{nukeindigo}{HTML}{7D3C98}
...
% ============================================================================
% PAGE STYLE - Clean academic style
% ============================================================================
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{%
  \begin{tikzpicture}[baseline=-0.5ex]
    \foreach \x in {0,0.12,0.24,0.36,0.48} {
      \foreach \y in {0,0.12,0.24} {
        \fill[academicblue, opacity=0.3] (\x,\y) circle (0.03);
      }
    }
  \end{tikzpicture}
  \hspace{0.5em}\textcolor{academicnavy}{\textsf{\small ETRA-2025-WMD-001}}%
}
\fancyhead[R]{\textcolor{softslate}{\textsf{\thepage}}}
\fancyfoot[C]{\textcolor{softslate}{\footnotesize\textsf{Emerging Technology Risk Assessment | Public Policy Research}}}
...
\titleformat{\section}
  ...
  {\thesection}{0.8em}{}[\vspace{-0.3em}{\color{medgray}\rule{\textwidth}{1pt}}]
...
% ============================================================================
% TCOLORBOX ENVIRONMENTS - Academic styling
% ============================================================================
\tcbuselibrary{skins,breakable,hooks}

\newtcolorbox{keybox}[1][Key Finding]{
  enhanced, breakable,
  colback=academicblue!8, colframe=academicblue,
  colbacktitle=academicblue, coltitle=white,
...
\newtcolorbox{contextbox}[1][Context]{
  enhanced, breakable,
  colback=researchgreen!8, colframe=researchgreen,
  colbacktitle=researchgreen, coltitle=white,
  fonttitle=\bfseries\sffamily,
...
\newtcolorbox{notebox}[1][Note]{
  enhanced, breakable,
  colback=warnambber!10, colframe=warnambber,
  colbacktitle=warnambber, coltitle=white,
  fonttitle=\bfseries\sffamily,
...
\newtcolorbox{infobox}[1][Information]{
  enhanced, breakable,
  colback=infoteal!8, colframe=infoteal,
  colbacktitle=infoteal, coltitle=white,
  fonttitle=\bfseries\sffamily,
  title={\ding{73}\hspace{0.5em}#1},
  boxrule=0pt, leftrule=3pt, arc=0pt, outer arc=0pt,
  left=12pt, right=12pt, top=8pt, bottom=8pt
...
% ============================================================================
% TITLE PAGE - Professional academic style
% ============================================================================
\begin{titlepage}
\begin{tikzpicture}[remember picture, overlay]
  % Background - clean gradient
  \fill[academicnavy] (current page.north west) rectangle (current page.south east);

  % Subtle grid pattern (right side)
  \begin{scope}[shift={(current page.center)}, opacity=0.06]
    \foreach \x in {2,2.6,...,10} {
      \foreach \y in {-12,-11.4,...,12} {
        \fill[white] (\x,\y) circle (0.05);
      }
    }
  \end{scope}

  % Main visualization - interconnected research domains
  \begin{scope}[shift={([xshift=-0.5cm, yshift=1.5cm]current page.center)}]
    ...
    \node[academicnavy, font=\fontsize{7}{7}\selectfont\bfseries] at (0,0.15) {POLICY};
    \node[academicnavy, font=\fontsize{7}{7}\selectfont\bfseries] at (0,-0.15) {ANALYSIS};

    % Domain nodes - softer academic colors
    \foreach \angle/\col/\label in {
      45/biogreen/BIO,
      90/academicblue/TECH,
      135/chemblue/CHEM,
      180/nukeindigo/NUKE,
      225/researchgreen/GOV,
      270/infoteal/INTL,
      315/softslate/ATTR,
      0/warnambber/DEF
    } {
      \fill[\col, opacity=0.85] (\angle:4.5cm) circle (0.4cm);
      \node[white, font=\fontsize{5}{5}\selectfont\bfseries] at (\angle:4.5cm) {\label};
      \draw[\col, opacity=0.3, line width=1pt] (0,0) -- (\angle:4cm);
      \foreach \r in {5.2,5.6,6} {
        \fill[\col, opacity=0.15] (\angle:\r cm) circle (0.08);
      }
    }

    % Inner ring
    \foreach \angle/\col in {0/academicblue, 45/biogreen, 90/researchgreen, 135/chemblue, 180/nukeindigo, 225/infoteal, 270/warnambber, 315/softslate} {
      \fill[\col, opacity=0.5] (\angle:2.5cm) circle (0.2cm);
      \draw[\col, opacity=0.2, line width=0.5pt] (\angle:2.5cm) -- ({(\angle+45)}:2.5cm);
    }
  \end{scope}

  % Document header bar
  \fill[accentgold] ([yshift=-1.5cm]current page.north west) rectangle ([yshift=-2.2cm]current page.north east);
  \node[academicnavy, font=\small\bfseries\sffamily] at ([yshift=-1.85cm]current page.north) {PROJECTION REPORT \textbar{} ETRA-2025-WMD-001 \textbar{} DECEMBER 2025};

  % Title block
  \node[anchor=south west, text width=14cm] at ([xshift=1.5cm, yshift=3cm]current page.south west) {
    {\fontsize{11}{13}\selectfont\color{accentgold}\sffamily EMERGING TECHNOLOGY RISK ASSESSMENT}\\[0.8cm]
  ...
  % Accent line
  \draw[accentgold, line width=2pt] ([xshift=1.5cm, yshift=3cm]current page.south west) -- ([xshift=1.5cm, yshift=10.5cm]current page.south west);
...
\noindent\begin{tikzpicture}
  \fill[academicnavy] (0,0) rectangle (\textwidth, 0.15);
  \foreach \x in {0.5,1,...,16.5} {
    \fill[accentgold, opacity=0.5] (\x, 0.075) circle (0.04);
  }
\end{tikzpicture}

\vspace{0.5cm}
\begin{center}
{\color{academicnavy}\Large\bfseries About This Document}
\end{center}
\vspace{0.3cm}

\begin{tcolorbox}[enhanced, colback=lightgray, colframe=academicblue!50, boxrule=1pt, arc=3pt,
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  \fill[academicnavy] (0,0) rectangle (\paperwidth, -10cm);

  \node[accentgold, font=\fontsize{10}{10}\selectfont\sffamily] at (0.5\paperwidth, -1.2cm) {PART I};
  ...
  % Simple academic chart
  \begin{scope}[shift={(3cm, -8cm)}]
    \draw[white, opacity=0.4, line width=1.2pt, -Stealth] (0,0) -- (6,0) node[right, font=\fontsize{7}{7}\selectfont, white, opacity=0.7] {TIME};
    \draw[white, opacity=0.4, line width=1.2pt, -Stealth] (0,0) -- (0,2.5) node[above, font=\fontsize{7}{7}\selectfont, white, opacity=0.7] {BARRIER};

    \foreach \y in {0.6,1.2,1.8} {
      \draw[white, opacity=0.1, line width=0.5pt] (0,\y) -- (5.8,\y);
    }

    % Bio barriers declining
    \draw[biogreen, opacity=0.8, line width=2pt] (0.3, 2.0) .. controls (2, 1.8) and (4, 1.2) .. (5.5, 0.8);
    \node[biogreen, font=\fontsize{6}{6}\selectfont] at (5.7, 0.9) {BIO};

    % Nuclear barriers stable
    \draw[nukeindigo, opacity=0.8, line width=2pt] (0.3, 2.2) -- (5.5, 2.1);
    \node[nukeindigo, font=\fontsize{6}{6}\selectfont] at (5.7, 2.2) {NUKE};

    \node[white, opacity=0.6, font=\fontsize{6}{6}\selectfont] at (1.0, -0.35) {2020};
    \node[white, opacity=0.8, font=\fontsize{6}{6}\selectfont\bfseries] at (3.0, -0.35) {2025};
    \node[white, opacity=0.6, font=\fontsize{6}{6}\selectfont] at (5.0, -0.35) {2030};
  \end{scope}

  % Legend
  \node[white, opacity=0.5, font=\fontsize{7}{7}\selectfont, anchor=west] at (10.5cm, -5.7cm) {BARRIER STRENGTH OVER TIME};
  \node[anchor=west] at (10.5cm, -6.4cm) {
    \begin{tikzpicture}
      \fill[biogreen, opacity=0.7] (0,0) circle (0.1);
      \node[white, opacity=0.7, font=\fontsize{7}{7}\selectfont, anchor=west] at (0.25, 0) {Biological - Declining};
    \end{tikzpicture}
  };
  \node[anchor=west] at (10.5cm, -7.0cm) {
    \begin{tikzpicture}
      \fill[nukeindigo, opacity=0.7] (0,0) circle (0.1);
      \node[white, opacity=0.7, font=\fontsize{7}{7}\selectfont, anchor=west] at (0.25, 0) {Nuclear - Stable (Strong)};
    \end{tikzpicture}
  };

  \fill[accentgold] (2cm, -9.2cm) rectangle (\paperwidth-2cm, -9.3cm);
\end{tikzpicture}
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \node[accentgold, font=\fontsize{10}{10}\selectfont\sffamily] at (0.5\paperwidth, -1.2cm) {PART II};
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \node[accentgold, font=\fontsize{10}{10}\selectfont\sffamily] at (0.5\paperwidth, -1.2cm) {PART III};
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \node[accentgold, font=\fontsize{10}{10}\selectfont\sffamily] at (0.5\paperwidth, -1.2cm) {PART IV};
