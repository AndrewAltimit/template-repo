...
\fancyhead[L]{%
  \begin{tikzpicture}[baseline=-0.5ex]
    % Hexagonal pattern (molecular/biosecurity feel)
    \foreach \i in {0,1,2} {
      \draw[scienceteal, opacity=0.4, line width=0.4pt]
        ({0.2+\i*0.35},0.12) -- ++(60:0.12) -- ++(0:0.12) -- ++(-60:0.12)
        -- ++(-120:0.12) -- ++(180:0.12) -- cycle;
    }
    \fill[barriergold, opacity=0.6] (0.32,0.12) circle (0.04);
  \end{tikzpicture}
  \hspace{0.3em}\textcolor{deepteal}{\textsf{\small ETRA-2025-WMD-001}}%
}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \begin{scope}[shift={([xshift=-0.3cm, yshift=1.2cm]current page.center)}]
    % Outer barrier ring (weakest - eroding)
    ...
    \fill[barriergold] (0,0) circle (0.7cm);
    \node[deepteal, font=\fontsize{6}{6}\selectfont\bfseries] at (0,0.1) {DEFENSE};
    \node[deepteal, font=\fontsize{6}{6}\selectfont\bfseries] at (0,-0.15) {IN DEPTH};

    % Barrier labels with domain icons
    \foreach \angle/\col/\label/\dist in {
      20/biogreen/BIO/5.5,
      70/labcyan/SCREEN/5.5,
      110/scienceteal/DETECT/4,
      160/chemblue/CHEM/4,
      200/nukepurple/NUKE/4,
      250/barriergold/ATTRIB/2.5,
      290/shieldamber/GOVERN/2.5,
      340/biogreen/INTL/5.5
    } {
      \fill[\col, opacity=0.9] (\angle:\dist cm) circle (0.35cm);
      \node[white, font=\fontsize{4}{4}\selectfont\bfseries] at (\angle:\dist cm) {\label};
    }

    % Breach indicators (showing where barriers are weakening)
    \foreach \angle in {45, 135, 225, 315} {
      \draw[white, opacity=0.3, line width=1pt, ->] (\angle:6.2cm) -- (\angle:5.8cm);
    }

    % Barrier strength indicators
    \node[white, opacity=0.5, font=\fontsize{5}{5}\selectfont] at (0, 6.2) {INFORMATION BARRIER};
    \node[white, opacity=0.5, font=\fontsize{5}{5}\selectfont] at (0, 4.7) {PHYSICAL BARRIER};
    \node[white, opacity=0.5, font=\fontsize{5}{5}\selectfont] at (0, 3.2) {TACIT KNOWLEDGE};
  \end{scope}
...
\noindent\begin{tikzpicture}
  \fill[deepteal] (0,0) rectangle (\textwidth, 0.15);
  % Hexagonal accent pattern
  \foreach \x in {0.5,1.3,...,16} {
    \draw[barriergold, opacity=0.6, line width=0.4pt]
      (\x, 0.075) -- ++(60:0.06) -- ++(0:0.06) -- ++(-60:0.06)
      -- ++(-120:0.06) -- ++(180:0.06) -- cycle;
  }
\end{tikzpicture}
...
\noindent\hspace*{-0.85in}\begin{tikzpicture}
  ...
  \begin{scope}[shift={(4cm, -7.5cm)}]
    ...
    \draw[labcyan, opacity=0.2, line width=6pt] (0,0) circle (2.2cm);
    \draw[labcyan, opacity=0.35, line width=1.5pt, dashed] (0,0) circle (2.2cm);
    \draw[scienceteal, opacity=0.3, line width=5pt] (0,0) circle (1.5cm);
    \draw[scienceteal, opacity=0.5, line width=1.5pt] (0,0) circle (1.5cm);
    \draw[barriergold, opacity=0.4, line width=4pt] (0,0) circle (0.8cm);
    \draw[barriergold, opacity=0.7, line width=1.5pt] (0,0) circle (0.8cm);
    \fill[barriergold, opacity=0.9] (0,0) circle (0.3cm);
    % Sector labels
    \node[white, opacity=0.6, font=\fontsize{5}{5}\selectfont] at (0, 2.6) {INFO};
    \node[white, opacity=0.6, font=\fontsize{5}{5}\selectfont] at (0, 1.85) {PHYS};
    \node[white, opacity=0.6, font=\fontsize{5}{5}\selectfont] at (0, 1.15) {TACIT};
  \end{scope}
