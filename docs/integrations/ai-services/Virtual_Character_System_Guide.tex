...
\begin{tikzpicture}[
  ...
  \node[flowbox=vcteal!70, below=0.5cm of fast] (emotion) {Emotion Classifier};
  \node[flowbox=vcteal!70, below=0.4cm of emotion] (filler) {Filler Generator};

  % Slow outputs - right column
  \node[flowbox=vcpurple!70, below=0.5cm of slow] (reason) {Claude Opus 4.5};
  \node[flowbox=vcpurple!70, below=0.4cm of reason] (memory) {Memory Query};
...
\begin{tabular}{@{}>{\raggedright}p{3.2cm}p{4.5cm}p{4.5cm}@{}}
\toprule
...
\midrule
MCP Server & \texttt{server.py} & \textcolor{successgreen}{\faCheck} Complete \\
Backend Interface & \texttt{backends/base.py} & \textcolor{successgreen}{\faCheck} Complete \\
VRChat Backend & \texttt{backends/vrchat\_remote.py} & \textcolor{successgreen}{\faCheck} Complete \\
Mock Backend & \texttt{backends/mock.py} & \textcolor{successgreen}{\faCheck} Complete \\
Audio Handler & \texttt{audio\_handler.py} & \textcolor{successgreen}{\faCheck} Complete \\
Sequence Handler & \texttt{sequence\_handler.py} & \textcolor{successgreen}{\faCheck} Complete \\
Blender Backend & \texttt{backends/blender.py} & \textcolor{warningamber}{Planned} \\
Unity Backend & \texttt{backends/unity.py} & \textcolor{warningamber}{Planned} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}p{2cm}p{2.8cm}p{2.8cm}p{3.8cm}@{}}
\toprule
...
\textbf{Reasoner} & Claude API & Qwen3, Gemma 2 & Implement \texttt{Reasoner} ABC \\
...
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}>{\raggedright}p{2.8cm}p{3.5cm}p{3.5cm}p{2.5cm}@{}}
\toprule
...
\begin{lstlisting}[language=Python, caption=Filler Selection]
FILLERS = {
...
def get_contextual_filler(emotion, vibe):
    if vibe == "somber":
        return "I understand..."
...
\begin{lstlisting}[language=Python, caption=Reasoner Abstract Base Class]
class Reasoner(ABC):
    @abstractmethod
...
\begin{lstlisting}[language=Python, caption=Emotion to PAD Vector Mapping]
EMOTION_TO_PAD = {
...
\begin{lstlisting}[language=Python, caption=Emotion Transition Manager]
class EmotionTransitionManager:
    def __init__(self):
        self.current_state = EmotionVector(0, 0, 0)
...
\begin{lstlisting}[language=Python, caption=Emotion to Audio Tags]
def get_audio_tags_for_emotion(emotion, intensity):
    if emotion == CanonicalEmotion.JOY:
        if intensity > 0.7:
            return ["[laughs]", "[excited]"]
        return ["[happy]"]
    elif emotion == CanonicalEmotion.SADNESS:
        return ["[sighs]", "[somber]"]
    # ... remaining mappings
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Emotion to Avatar Config]
def emotion_to_avatar(emotion):
    mapping = {
        CanonicalEmotion.JOY: {"emotion": "happy", "gesture": "cheer"},
        CanonicalEmotion.SADNESS: {"emotion": "sad", "gesture": None},
        CanonicalEmotion.THINKING: {"emotion": "neutral", "gesture": "thinking"},
        # ... remaining mappings
    }
    return mapping.get(emotion, {"emotion": "neutral", "gesture": None})
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Audio Handler Features]
class AudioHandler:
    """Async audio processing with security"""

    async def process_audio(self, audio_data: str) -> AudioResult:
        """
...
\begin{lstlisting}[language=Python, caption=OSC Message Examples]
# Avatar expressions
...
\begin{lstlisting}[language=Python, caption=VRChat Remote Backend]
class VRChatRemoteBackend(BackendAdapter):
    """VRChat backend via OSC-over-HTTP bridge"""

    async def send_animation(self, data: CanonicalAnimationData):
        osc_messages = []

        # Emotion parameters
        if data.emotion:
            osc_messages.append({
                "address": f"/avatar/parameters/Emotion_{data.emotion}",
                "value": data.emotion_intensity
            })

        # Gesture
        if data.gesture:
            gesture_index = GESTURE_MAP.get(data.gesture, 0)
            osc_messages.append({
...
\begin{lstlisting}[language=Python, caption=Creating a Sequence]
# Create sequence
...
await virtual_character.add_sequence_event(
    ...
    timestamp=0.0,
    parallel_events=[
        {"event_type": "expression", "expression": "happy", "intensity": 0.8},
        {"event_type": "gesture", "gesture": "wave"},
        {"event_type": "audio", "audio_data": "outputs/greeting.mp3"}
    ]
)

# Play
await virtual_character.play_sequence()
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Namespace Organization]
personality/
...
\begin{lstlisting}[language=Python, caption=Storing Voice Preferences]
...
\begin{lstlisting}[language=Python, caption=Pattern Compilation]
...
\begin{lstlisting}[language=Python, caption=Multi-Agent Coordination Example]
class MultiAgentCoordinator:
    """Manages multiple agents in shared virtual space."""

    async def spawn_agents(self, count: int, world_id: str):
        """Spawn N agents into the same virtual world."""
        for i in range(count):
            agent = AgentInstance(f"agent_{i}")
            await agent.connect(backend="vrchat", world=world_id)
            self.agents.append(agent)
            await self.broadcast_event("agent_joined", agent.id)

    async def coordinate_interaction(self, agent_a: str, agent_b: str):
        """Coordinate turn-taking between two agents."""
        # Agent A speaks, Agent B listens (shows listening expression)
        # When A finishes, B responds (emotion-appropriate reaction)
        pass
\end{lstlisting}
...
\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}>{\raggedright}p{3cm}p{4cm}p{4.5cm}@{}}
\toprule
...
\begin{lstlisting}[language=Python, caption=Expression Orchestrator]
class ExpressionOrchestrator:
    ...
    async def express(
        ...
        if "reaction" in modalities:
            ...
            results["reaction"] = await self.reaction_search.search(query)
...
\begin{lstlisting}[language=Python, caption=Complete Response Flow]
async def respond_with_embodiment(input_text: str):
    ...
    await expression_orchestrator.express(
        text=synthesis.response,
        emotion=synthesis.emotion,
        modalities=["voice", "avatar", "reaction"]
    )
\end{lstlisting}
...
\midrule
MCP Middleware & Linux Dev Machine & Python 3.11+ & 8020 \\
...
\begin{lstlisting}[language=Python, caption=Common OSC Messages]
...
\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}>{\raggedright}p{4cm}p{4cm}p{4cm}@{}}
\toprule
...
\begin{enumerate}[nosep, leftmargin=1.5em]
  \item[\ding{111}] \textbf{Linux Machine}: Python 3.11+, pip, Docker installed
  ...
  \item[\ding{111}] \textbf{Test}: Run \texttt{python scripts/test\_connection.py}
\end{enumerate}
\end{tcolorbox}
