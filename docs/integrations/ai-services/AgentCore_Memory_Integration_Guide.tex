...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Key metrics boxes
  \node[fill=lightgray, rounded corners=6pt, minimum width=3cm, minimum height=2cm] (m1)
    at ([xshift=9.5cm, yshift=-17cm]current page.north west) {};
  \node[awsorange, font=\large] at ([yshift=0.35cm]m1.center) {\faDatabase};
  \node[awsblue, font=\LARGE\bfseries] at (m1.center) {3};
  \node[coolgray, font=\scriptsize] at ([yshift=-0.45cm]m1.center) {Memory Providers};

  \node[fill=lightgray, rounded corners=6pt, minimum width=3cm, minimum height=2cm] (m2)
    at ([xshift=13.3cm, yshift=-17cm]current page.north west) {};
  \node[dangerred, font=\large] at ([yshift=0.35cm]m2.center) {\faCog};
  \node[awsblue, font=\LARGE\bfseries] at (m2.center) {0.25};
  \node[coolgray, font=\scriptsize] at ([yshift=-0.45cm]m2.center) {req/sec (events)};

  \node[fill=lightgray, rounded corners=6pt, minimum width=3cm, minimum height=2cm] (m3)
    at ([xshift=17.1cm, yshift=-17cm]current page.north west) {};
  \node[successgreen, font=\large] at ([yshift=0.35cm]m3.center) {\faRobot};
  \node[awsblue, font=\LARGE\bfseries] at (m3.center) {5+};
  \node[coolgray, font=\scriptsize] at ([yshift=-0.45cm]m3.center) {AI Agents};

  % Feature highlights
  \node[fill=cloudblue!10, rounded corners=4pt, inner sep=8pt]
    at ([xshift=9.5cm, yshift=-20.5cm]current page.north west) {
  ...
  \node[fill=successgreen!10, rounded corners=4pt, inner sep=8pt]
    at ([xshift=13.5cm, yshift=-20.5cm]current page.north west) {
  ...
  \node[fill=awsorange!10, rounded corners=4pt, inner sep=8pt]
    at ([xshift=17.5cm, yshift=-20.5cm]current page.north west) {
  ...
  \node[fill=dangerred!10, rounded corners=3pt, inner sep=6pt]
    at ([xshift=10cm, yshift=-23cm]current page.north west) {
  ...
  \node[fill=successgreen!10, rounded corners=3pt, inner sep=6pt]
    at ([xshift=14cm, yshift=-23cm]current page.north west) {
    \textcolor{successgreen}{\faCheck}\hspace{0.3em}\scriptsize\sffamily No Limit
  };

  % Version info
  \node[coolgray, font=\sffamily] at ([xshift=13cm, yshift=-25cm]current page.north west) {
...
\subsection{PostgreSQL + pgvector Provider}

For production self-hosting with enterprise features:

\begin{lstlisting}[caption={PostgreSQL with pgvector for self-hosted production}]
import asyncpg
from sentence_transformers import SentenceTransformer

class PostgresProvider(MemoryProvider):
    """Production-grade self-hosted with SQL benefits."""
...
\begin{itemize}
  \item \textbf{Development}: ChromaDB (zero cost, no rate limits)
  \item \textbf{Self-hosted production}: PostgreSQL + pgvector (SQL, backups)
  \item \textbf{Enterprise/managed}: AWS AgentCore (fully managed, IAM)
\end{itemize}
...
\begin{tikzpicture}[
  ...
  step/.style={rectangle, rounded corners=4pt, fill=awsorange!10, draw=awsorange,
  ...
  \node[step] (recall) at (4.5, 0.5) {Recall similar issues};
  \node[step] (process) at (4.5, -0.5) {Process with context};
  \node[step] (store) at (9, 0) {Store learnings};
...
|       +-- postgres_provider.py  # PostgreSQL+pgvector
+-- scripts/
...
response = client.create_memory(
    ...
    eventExpiryDuration="P7D",  # ISO 8601: 7 days
)
...
class AsyncWriteQueue:
    ...
    FIX: Lock prevents race condition where multiple
