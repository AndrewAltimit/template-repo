#!/usr/bin/env python3
"""
Enhanced Company API Translation Wrapper with Automatic Tool Support Detection
    ...
    try:
        ...
        if not supports_tools and data.get("tools") and content:
            ...
            if parsed_tool_calls:
                ...
                # Remove tool call text from content if found
                # This is a simple approach - could be enhanced
                for pattern in [r"```tool_call.*?```", r"<tool>.*?</tool>"]:
                    import re
...
def health_check():
    ...
    return (
        jsonify(
            {
                "status": "healthy",
                "service": "enhanced_translation_wrapper",
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Enhanced Translation Wrapper on port {port}")
