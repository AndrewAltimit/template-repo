#!/bin/bash
# OpenCode wrapper with models.dev interception
# This intercepts both API calls AND the models list
...
if [ "$USE_PROXY" = "true" ]; then
    echo -e "${BLUE}Starting proxy services with interception...${NC}"
    echo ""
    ...
    if ! check_service 8052 "API Translation Wrapper"; then
        echo "Starting API Translation Wrapper with models.dev interception..."
    ...
    # Set up models.dev interception
    echo ""
    echo -e "${CYAN}Setting up models.dev interception...${NC}"

    # Check if we can modify /etc/hosts
    if [ -w /etc/hosts ]; then
        # Remove any existing models.dev entry
        grep -v "models.dev" /etc/hosts > /tmp/hosts.tmp || true
        # Add our interception
        echo "127.0.0.1 models.dev" >> /tmp/hosts.tmp
        cat /tmp/hosts.tmp > /etc/hosts
        echo -e "${GREEN}✓${NC} models.dev redirected to localhost:8052"
    else
        echo -e "${YELLOW}⚠${NC} Cannot modify /etc/hosts (need root). Models list won't be intercepted."
        echo "   To enable interception, run container with --privileged or as root"
    fi

    # Clear OpenCode's cached models to force refresh
    if [ -f ~/.cache/opencode/models.json ]; then
        rm ~/.cache/opencode/models.json
        echo -e "${GREEN}✓${NC} Cleared OpenCode model cache"
    fi

    echo ""
    echo -e "${GREEN}✓ Proxy services ready with interception${NC}"
    echo ""
...
EOF
    ...
    echo -e "${CYAN}Press Enter to start OpenCode with intercepted models list...${NC}"
    read -r
